...
- Specs start **logged out**: `cy.resetState()` reseeds the shard's own demo backend (demo profile) or the real database (live profile). Call `cy.loginAs('owner')` when a spec needs elevated visibility — that account holds the shop's `owner` role and the installation's `operator` role, so it reaches both the shop screens and the observability dashboard.
