...
**12m54s** on one core; `scripts/e2e/run-shards.ts` splits them across processes sharing one preview server.
...
- **The live profile is never sharded.** `cy.restore()` re-seeds the paired backend's real
...
- Specs start **logged out**: `cy.restore()` puts the shard's own demo backend back (demo profile) or re-seeds the real database (live profile). Call `cy.loginAs('owner')` when a spec needs elevated visibility — that account holds the shop's `owner` role and the installation's `operator` role, so it reaches both the shop screens and the observability dashboard.
