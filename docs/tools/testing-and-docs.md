...
| `tests/support/mocks/generated.ts` | FE | **Orval output** — one faker factory per operation, regenerated by `npm run genapi`, never edited. Raw material for the random profile above, not consumed directly by handlers |
