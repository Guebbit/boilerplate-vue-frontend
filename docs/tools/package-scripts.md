...
| Script                 | Job                                                                                                                                    | Read more                                        |
| ---------------------- | -------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------ |
| `regenerate`           | **After every pull.** `gen:api`, `gen:asyncapi`, then `prettier:fix` — the whole client rebuilt from the specs the backend handed over | [OpenAPI Workflow](../api/openapi-workflow.md)   |
...
Those two are the whole list — `SHARED_FILES` in the backend's `scripts/pairing/spec-identity.ts` is what both the copy and the gate read. `boilerplate-node-backend/db/demo/demo-data.json` is **not** on it: this repo keeps no copy, and the e2e suites seed from the backend's own fixtures instead.
