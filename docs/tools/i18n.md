...
| `api` | the API can *answer* in it | the UI falls back per key, but every error message is in the right language |

Both belong in the switcher, and **neither is validated against the other**. This app can bundle
`it.json` while the API has no Italian at all. That inconsistency is a *human* decision — someone
...
Its overrides — the `api`-scoped rows — are layered onto its files *inside* the API and never leave
