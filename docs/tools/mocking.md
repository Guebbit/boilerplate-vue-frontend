...
1. **Data parity.** The seed records in `tests/mocks/shared/mockShared.ts` mirror the backend's `db/seeds/index.ts` — same ids, same credentials, same content. This is what lets `cy.loginAs('user')` (`gino@pino.it` / `password`) work identically against MSW and against the real API.
