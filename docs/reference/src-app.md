...
| `src/app/components/AppNavigation.vue`         | The app shell's navigation: the labelled bar, the account and administration menus, the `pinned` buttons beside the account menu and the right-hand phone drawer, all built from the registry's navigation entries — grouped by `section` — rather than a hand-written list. | [Sitemap & Access Control](../theory/sitemap.md#navigation-sections)                                |
| `src/app/components/AppNavBarLink.vue`         | One desktop-bar entry: glyph plus label in full, optional count badge. The visible text is the accessible name.                                                                                                                                                              | [Sitemap & Access Control](../theory/sitemap.md#navigation-sections)                                |
| `src/app/components/AppNavPinnedButton.vue`    | A `pinned` entry as the bar shows it: glyph, count badge and a live detail text (the cart's total), with one accessible name carrying all three.                                                                                                                             | [Sitemap & Access Control](../theory/sitemap.md#navigation-sections)                                |
| `src/app/components/StaticPageLinks.vue`       | The cross-links at the bottom of every prose page — the other three of `STATIC_PAGES`, so the four pages cannot drift on what they link.                                                                                                                                     | [Sitemap & Access Control](../theory/sitemap.md)                                                    |
...
| `src/app/components/AppNavIconButton.vue`      | An icon-only button that still has a name — the account and administration menu activators: `aria-label` and a focus-openable, Escape-dismissable tooltip carry the same text.                                                                                               | [Accessibility testing](../tools/accessibility-testing.md)                                          |
...
| `src/app/components/AppVerificationBanner.vue` | The “confirm your email” banner, shown to a signed-in visitor whose address is unproved. In the shell rather than the profile page because the checkout refuses such an address.                                                                                             | [account](../modules/account.md)                                                                    |
...
| `src/app/views/AboutPage.vue`                  | The shop's About page: intro, a feature grid, the stack under the hood and a walkthrough with CTAs. Structure and icons live here, every word under `static-pages.about.*`.                                                                                                  | [Sitemap & Access Control](../theory/sitemap.md)                                                    |
| `src/app/views/FaqPage.vue`                    | The FAQ: topics of question/answer panels plus a contact CTA guarded by `hasRoute`. Topic keys live here, the copy under `static-pages.faq.topics.*`.                                                                                                                        | [Sitemap & Access Control](../theory/sitemap.md)                                                    |
