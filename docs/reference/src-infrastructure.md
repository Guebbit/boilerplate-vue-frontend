...
`src/infrastructure/` is the substrate: everything the application runs *on*, and nothing about
...
| `src/infrastructure/observability/analytics-events.ts` | **Generated in the paired backend, byte-identical here.** The analytics event names this app emits — the client-only half of one shared namespace, so a name cannot be claimed by both sides. | [Umami](../tools/umami.md) · [Contracts](./contracts.md) |
