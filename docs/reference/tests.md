...
*which* test covers a rule is most of the value of having it.
...
|---|---|---|
| `tests/e2e/visual/__snapshots__/*.png` | The committed visual baselines — what a page is *supposed* to look like. A failure diff is never committed: recording a failure as expected output is how a regression becomes the baseline. | [Visual Regression](../tools/visual-regression.md) |
