...
| Session hijacking / sidejacking (partial)            | the one long-lived credential (`HttpOnly` refresh cookie) never reaches JS at all, on this side or an attacker's                                                                         | `infrastructure/session.ts`, `boilerplate-node-backend/src/modules/account/session/jwt.ts`       |
...
| Permissive CORS (§13/§2) — **shared**                             | this app has no CORS configuration of its own (a browser loading `index.html` needs none) but sets `withCredentials: true` on every API call — which is what makes the backend's explicit origin allowlist load-bearing rather than a nicety: a credentialed request from an unlisted origin is refused by the BROWSER, reading the backend's own CORS response             | `infrastructure/http/client.ts`, `boilerplate-node-backend/src/app/security.ts`                                |
...
| Spam via forms (§21) — **shared**                      | the contact form's honeypot FIELD — `aria-hidden`, `tabindex="-1"`, invisible to a sighted human and to a screen reader, but present in the DOM for a bot that fills every field — is built here; deciding what a non-empty value MEANS (mark as spam, skip the notification, still answer 201) is the backend's | `modules/feedback/views/Contact.vue`, `boilerplate-node-backend/src/modules/feedback/service.ts#create` |
