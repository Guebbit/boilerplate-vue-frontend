...
/**
 * Whether the invoice PDF is still being generated by the async worker — `GET /orders/:id/invoice`
 * would only answer `202` right now, so the download button stays disabled until this clears on
 * its own (see {@link usePollInvoiceStatus}). `undefined` (an order that predates the async
 * pipeline) reads as NOT pending: that order's invoice has always rendered on demand.
 *
 * @returns `true` while the button should stay disabled for this reason.
 */
