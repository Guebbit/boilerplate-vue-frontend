...
>(initialData: T = {} as T, schema?: ZodType<T>) => {
    /**
     * Snapshot of the baseline form data, updated by setInitial.
     * Used by isDirty to detect unsaved changes.
     */
    const initialDataRef = ref<T>({ ...initialData }) as Ref<T>;
    ...
    const isDirty = computed(
        () => JSON.stringify(form.value) !== JSON.stringify(initialDataRef.value)
    );
     ...
     */
    const resetForm = () => {
        form.value = { ...initialDataRef.value };
        formErrors.value = {};
    };
    ...
    /**
     * Set new baseline data for dirty-checking and populate the form.
     * Accepts either a plain value or a Vue ref (e.g. from storeToRefs).
     *
     * @param data
     */
    const setInitial = (data: T | Ref<T | undefined>) => {
        const value = isRef(data) ? data.value : data;
        if (!value) return;
        initialDataRef.value = { ...value };
        form.value = { ...value };
    };
