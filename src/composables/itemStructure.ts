...
export const useItemStructure = <T = unknown>(itemIdentifier = 'id') => {
    /**
    ...
    const editRecord = (id: string | number, data: Partial<T>) => {
        const index = itemList.value.findIndex(
            (item) => (item as T)[itemIdentifier as keyof T] === id
        );
        if (index !== -1) itemList.value[index] = { ...itemList.value[index], ...data };
    };
