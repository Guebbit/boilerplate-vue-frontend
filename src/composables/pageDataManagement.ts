...
export const usePageDataManagement = () => {
    ...
    const { fetchRun } = useRunsStore();
    const { selectedRunId } = storeToRefs(useRunsStore());

    const { fetchEvidences, fetchEvidence } = useEvidencesStore();
    const { selectedEvidenceId } = storeToRefs(useEvidencesStore());
    ...
    const pageTargetProject = (
        ...
        finalRoute = () => {},
        catchRoute: (error: unknown) => void = () => {}
    ) => {
        // Select and fetch target data
        const selectAndFetch = () => {
            selectedProjectId.value = projectId.value;
            return fetchProject(projectId.value).catch(catchRoute).finally(finalRoute);
        };
        // Trigger on ref change and on mount
        watch(() => projectId.value, selectAndFetch);
    ...
    const pageTargetRun = (
        ...
        // Trigger on ref change and on mount
        watch([() => projectId.value, () => runId.value], selectAndFetch);
        onMounted(selectAndFetch);
    };
    ...
    const pageTargetEvidence = (
        ...
        // Trigger on ref change and on mount
        watch([() => projectId.value, () => runId.value, () => eventId.value], selectAndFetch);
        onMounted(selectAndFetch);
    };
    ...
    const pageTargetUser = (
        ...
        const selectAndFetch = () => {
            ...
            return fetchUser(userId.value).catch(catchRoute).finally(finalRoute);
        };
        // Trigger on ref change and on mount
        watch(() => userId.value, selectAndFetch);
        onMounted(selectAndFetch);
    };
