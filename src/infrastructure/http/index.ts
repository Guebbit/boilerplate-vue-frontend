 ...
 * dictionary happens to be loaded — see `@/infrastructure/i18n.ts`.
 *
...
const validateResponseAgainstContract = (config: AxiosRequestConfig, data: unknown): void => {
    // An unmapped endpoint warns rather than throws — a gap in the map is not a bad response.
...
export const orvalMutator = <T>(
    ...
    // `config` wins over `options`: the generated call site is more specific than the caller's base.
