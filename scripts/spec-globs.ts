 ...
 * `scripts/e2e-shard.ts` (what the default gate schedules), and five `--spec` arguments in
 * `package.json`. Three of them read the constants below; `package.json` cannot import anything, so
 * `tests/unit/scripts/spec-globs.spec.ts` asserts its strings resolve to the same files instead.
 *
