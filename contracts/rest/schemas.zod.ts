...
export const getEntityTranslationsResponseDataTranslationsItemLocaleRegExp = new RegExp('^[a-z]{2}(-[A-Za-z0-9]+)*$');
...
export const GetEntityTranslationsResponse = zod.strictObject({
  ...
  "message": zod.string(),
  "data": zod.strictObject({
  "entityType": zod.string().min(1).describe('A `translatables` registry key — `product` in V1. Not an enum: the set grows as more of the kernel registry declares translatable collections, and this contract does not enumerate a kernel manifest. An unregistered value is refused at write time with a 422, not by this schema.'),
  "entityId": zod.string().describe('Resource identifier'),
  "translations": zod.array(zod.strictObject({
  "id": zod.string().describe('Resource identifier'),
  "entityType": zod.string().min(1).describe('A `translatables` registry key — `product` in V1. Not an enum: the set grows as more of the kernel registry declares translatable collections, and this contract does not enumerate a kernel manifest. An unregistered value is refused at write time with a 422, not by this schema.'),
  "entityId": zod.string().describe('Resource identifier'),
  "locale": zod.string().regex(getEntityTranslationsResponseDataTranslationsItemLocaleRegExp).describe('BCP 47 language tag, e.g. `en` or `it`. Which tags a deployment actually supports is a runtime fact, not a contract one — ask `GET \/locales`.'),
  "fields": zod.record(zod.string(), zod.string()).describe('Field name to translated value — `{ title, description }` for a product. Keys are validated against the `translatables` registry entry for `entityType`, never against this contract, since the field set differs per entity and this schema stays generic on purpose.'),
  "sourceDigest": zod.string().optional().describe('Hash of the fallback-locale row\'s `fields` at the moment this row was translated. A mismatch means the source has changed since — stale, not wrong. Absent on the fallback-locale row itself, which is never stale relative to its own content.'),
  "origin": zod.enum(['machine', 'human']).describe('What produced this row\'s current `fields` — what a reviewer needs to prioritise. There is no workflow gate behind it: a `human` write is live immediately, same as a `machine` one.'),
  "translatedBy": zod.string().describe('Resource identifier').optional().describe('The `translator` who last wrote this row, for the audit trail.'),
  "createdAt": zod.iso.datetime({"offset":true}),
  "updatedAt": zod.iso.datetime({"offset":true})
}).describe('One entity\'s words in one language. Unique on (entityType, entityId, locale) — the fallback-locale row is not a special case, it is simply the row whose `locale` equals this deployment\'s `NODE_FALLBACK_LOCALE`.')),
  "fields": zod.array(zod.string()).describe('Every field name the `translatables` registry declares for this `entityType` — the field set the generic translation screen offers, so it is never limited to whatever fields the fetched rows happen to carry already.')
}).describe('Every locale one entity has a row for, in one response — the shape the admin translation screen for that entity reads and writes as a whole.')
})
...
export const UpsertEntityTranslationsBody = zod.record(zod.string(), zod.strictObject({
  "fields": zod.record(zod.string(), zod.string()).describe('Every key MUST be one the `translatables` registry declares for this `entityType`; an unknown key is a 422, not a silently dropped one. Empty is a 422 too — enforced at write time, not by this schema, since `minProperties` is not something the generated client validates.'),
  "origin": zod.enum(['machine', 'human']).describe('What produced this row\'s current `fields` — what a reviewer needs to prioritise. There is no workflow gate behind it: a `human` write is live immediately, same as a `machine` one.').default(upsertEntityTranslationsBodyOneOriginDefault)
}).nullable().describe('Upserts this locale\'s row when an object, deletes it when `null`. A key this map does not name is left exactly as it is — see the operation description for the full three-way table.')).describe('One or more locales for one entity, keyed by locale tag. A key absent from this map leaves that locale untouched; see the operation description for what an object and a `null` each mean.')



export const upsertEntityTranslationsResponseDataTranslationsItemLocaleRegExp = new RegExp('^[a-z]{2}(-[A-Za-z0-9]+)*$');
...
export const UpsertEntityTranslationsResponse = zod.strictObject({
  ...
  "entityType": zod.string().min(1).describe('A `translatables` registry key — `product` in V1. Not an enum: the set grows as more of the kernel registry declares translatable collections, and this contract does not enumerate a kernel manifest. An unregistered value is refused at write time with a 422, not by this schema.'),
  "entityId": zod.string().describe('Resource identifier'),
  "locale": zod.string().regex(upsertEntityTranslationsResponseDataTranslationsItemLocaleRegExp).describe('BCP 47 language tag, e.g. `en` or `it`. Which tags a deployment actually supports is a runtime fact, not a contract one — ask `GET \/locales`.'),
  "fields": zod.record(zod.string(), zod.string()).describe('Field name to translated value — `{ title, description }` for a product. Keys are validated against the `translatables` registry entry for `entityType`, never against this contract, since the field set differs per entity and this schema stays generic on purpose.'),
  "sourceDigest": zod.string().optional().describe('Hash of the fallback-locale row\'s `fields` at the moment this row was translated. A mismatch means the source has changed since — stale, not wrong. Absent on the fallback-locale row itself, which is never stale relative to its own content.'),
  "origin": zod.enum(['machine', 'human']).describe('What produced this row\'s current `fields` — what a reviewer needs to prioritise. There is no workflow gate behind it: a `human` write is live immediately, same as a `machine` one.'),
  "translatedBy": zod.string().describe('Resource identifier').optional().describe('The `translator` who last wrote this row, for the audit trail.'),
...
export const GetObservabilityHealthResponse = zod.strictObject({
  ...
  "jobs": zod.array(zod.strictObject({
  "name": zod.string(),
  "lastSuccessAt": zod.iso.datetime({"offset":true}).nullish(),
  "lastError": zod.string().nullish()
}).describe('One lease-guarded job\'s last observed outcome, read off its `leases` document (`src\/infrastructure\/persistence\/lease.ts`). Absent `lastSuccessAt` means the job has never completed since its lease document was created; absent `lastError` means its most recent attempt did not fail.\nNot part of `status`, same reasoning as `telemetry`: a job that has not run recently costs staleness, not this instance\'s ability to serve a request — see docs\/reference\/ops.md#scheduled-jobs.')).optional().describe('Every lease-guarded job that has attempted to run at least once. See `ObservabilityHealthJob`.'),
...
export const GetObservabilityAuditLogsResponse = zod.strictObject({
  ...
  "actor_role_name": zod.string().optional().describe('The tenant role behind actor_role, e.g. moderator. Absent when the request never resolved one.'),
  "actor_scope": zod.enum(['tenant', 'platform']).optional().describe('Which world this action happened in — a shop or the installation. Absent on a row recorded before this field existed.'),
...
export const ListAuditEntriesQueryParams = zod.strictObject({
  ...
  "action": zod.string().optional().describe('Filter by action name (e.g. order.created)'),
  "outcome": zod.enum(['success', 'failure']).optional().describe('Filter by outcome'),
  "target": zod.string().optional().describe('Filter by the id of the row the action was taken on (e.g. an order or a user)'),
  "since": zod.iso.datetime({"offset":true}).optional().describe('Return events strictly after this ISO-8601 timestamp — an exclusive bound'),
  "page": zod.number().min(1).max(listAuditEntriesQueryPageMax).default(listAuditEntriesQueryPageDefault).describe('1-based page index'),
  "pageSize": zod.number().min(1).max(listAuditEntriesQueryPageSizeMax).default(listAuditEntriesQueryPageSizeDefault)
})
...
export const ListAuditEntriesResponse = zod.strictObject({
  ...
  "meta": zod.strictObject({
  "page": zod.number().min(1).max(listAuditEntriesResponseDataMetaPageMax).default(listAuditEntriesResponseDataMetaPageDefault).describe('1-based page index. Bounded so page × pageSize cannot ask for an unbounded Mongo skip.'),
  "pageSize": zod.number().min(1).max(listAuditEntriesResponseDataMetaPageSizeMax).default(listAuditEntriesResponseDataMetaPageSizeDefault).describe('Optional override; server may clamp to a max'),
...
export const CheckPasswordBreachedBody = zod.strictObject({
  "password": zod.string().min(1).describe('A CANDIDATE password, not `PasswordNew` — this runs while someone is still typing, so it accepts anything non-empty rather than the contract\'s own composition rule.')
})
...
export const CheckPasswordBreachedResponse = zod.strictObject({
  ...
  "message": zod.string(),
  "data": zod.strictObject({
  "breached": zod.boolean().describe('Whether the candidate matches a known-breached password — the bundled list or, when enabled, the HIBP range lookup. Advisory: the four password-SET paths are the actual gate, not this field.'),
  "count": zod.number().min(checkPasswordBreachedResponseDataCountMin).optional().describe('How many times HIBP has seen this exact password. Present only when HIBP is what caught it — never for a bundled-list hit, and never when rung 2 is disabled or unreachable.')
})
})
...
export const RequestEmailVerificationResponse = zod.strictObject({
  ...
  "message": zod.string(),
  "data": zod.strictObject({
  "resendAfter": zod.number().describe('Seconds before another verification email may be requested. A client counts down from this rather than inventing its own cooldown, so it never disagrees with the server.')
})
})
...
export const SignupHeader = zod.strictObject({
  ...
  "Idempotency-Key": zod.string().min(1).max(signupHeaderIdempotencyKeyMax).regex(signupHeaderIdempotencyKeyRegExp).optional().describe('An opaque, client-generated value (a UUID by convention) that makes a retried write safe. Repeating this request with the SAME key and the SAME body replays the first response (`Idempotent-Replay: true`, no repeated write) instead of running it again; the same key with a DIFFERENT body answers 422; a key still being processed by another in-flight request answers 409. Omitting the header simply forgoes replay protection — the write still happens normally.\n')
})
...
export const exportAccountDataResponseDataOrdersItemItemsItemLocaleRegExp = new RegExp('^[a-z]{2}(-[A-Za-z0-9]+)*$');
...
export const ExportAccountDataResponse = zod.strictObject({
  ...
  "roles": zod.array(zod.strictObject({
  "role": zod.string(),
  ...
  "id": zod.string().describe('Resource identifier'),
  "title": zod.string(),
  "price": zod.number().min(exportAccountDataResponseDataOrdersItemItemsItemProductPriceMin).describe('Gross — what the customer paid, VAT included, frozen at checkout. Same convention as `Product.price`.'),
  ...
  "weight": zod.number().min(exportAccountDataResponseDataOrdersItemItemsItemProductWeightMin).optional(),
  "categories": zod.array(zod.string()).optional(),
  "tags": zod.array(zod.string()).optional(),
  "createdAt": zod.iso.datetime({"offset":true}).optional(),
  "updatedAt": zod.iso.datetime({"offset":true}).optional(),
  "deletedAt": zod.iso.datetime({"offset":true}).optional(),
  "taxRate": zod.number().min(exportAccountDataResponseDataOrdersItemItemsItemProductTaxRateMin).max(exportAccountDataResponseDataOrdersItemItemsItemProductTaxRateMax).optional().describe('The VAT rate this line was actually charged, as a decimal (0.22 for 22%). Frozen at checkout, same reasoning as `price`. Absent on an order placed before VAT existed.')
}),
  "quantity": zod.number().min(1),
  "locale": zod.string().regex(exportAccountDataResponseDataOrdersItemItemsItemLocaleRegExp).describe('BCP 47 language tag, e.g. `en` or `it`. Which tags a deployment actually supports is a runtime fact, not a contract one — ask `GET \/locales`.'),
  "current": zod.strictObject({
  "imageUrl": zod.string().describe('Absolute URL or server-relative upload path (e.g. `\/uploads\/abc.jpg`). `uri-reference`, not `uri`: an uploaded image is stored and returned as a path relative to the API host, which is not a valid absolute URI.'),
  "thumbnailUrl": zod.string().optional().describe('Server-relative path to a small WebP derivative of `imageUrl`, produced by the image digest pipeline once an uploaded image has finished processing (see `docs\/tools\/image-processing.md`). Absent for a record whose image is a remote or default URL rather than an upload — there is nothing to derive a thumbnail from. Never accepted on a request body: the server is the only writer.')
}).nullable().describe('The product\'s picture, resolved live — `null` when the catalogue product (`product.id`) has been hard-deleted. Never the terms of the sale, so it is never frozen; see `OrderLineCurrent`.'),
  "taxAmount": zod.number().min(exportAccountDataResponseDataOrdersItemItemsItemTaxAmountMin).optional().describe('VAT included in this line\'s total, at its own frozen `taxRate`.'),
  "netAmount": zod.number().min(exportAccountDataResponseDataOrdersItemItemsItemNetAmountMin).optional().describe('This line\'s total excluding VAT — `price × quantity` minus `taxAmount`.')
})),
  ...
  "netTotal": zod.number().min(exportAccountDataResponseDataOrdersItemNetTotalMin).optional().describe('Sum of every line\'s `netAmount` — the goods total excluding VAT, GOODS ONLY. Shipping\'s own net amount is `shippingNetAmount`, not folded in here. Absent on a pre-VAT order.'),
  "taxTotal": zod.number().min(exportAccountDataResponseDataOrdersItemTaxTotalMin).optional().describe('Every VAT collected on this order: the lines\' own `taxAmount`, plus the VAT on `shippingCost` — apportioned pro-rata across the lines by value and taxed at each line\'s own rate, since delivery is taxed as ancillary to what it delivers. Absent on a pre-VAT order.'),
  "shippingNetAmount": zod.number().min(exportAccountDataResponseDataOrdersItemShippingNetAmountMin).optional().describe('Shipping\'s own net amount, summed across every line it was apportioned onto — `netTotal` stays goods-only, this is the rest of the split. Absent on a pre-VAT order.'),
  "shippingTaxAmount": zod.number().min(exportAccountDataResponseDataOrdersItemShippingTaxAmountMin).optional().describe('Shipping\'s own tax amount, summed across every line it was apportioned onto — already folded into `taxTotal`, published separately so an invoice can print it as its own line. Absent on a pre-VAT order.'),
  "taxSummary": zod.array(zod.strictObject({
  "rate": zod.number().min(exportAccountDataResponseDataOrdersItemTaxSummaryItemRateMin).describe('The decimal rate this row is for (`0.22` for 22%) — never repeated across rows.'),
  "netAmount": zod.number().min(exportAccountDataResponseDataOrdersItemTaxSummaryItemNetAmountMin),
  "taxAmount": zod.number().min(exportAccountDataResponseDataOrdersItemTaxSummaryItemTaxAmountMin),
  "grossAmount": zod.number().min(exportAccountDataResponseDataOrdersItemTaxSummaryItemGrossAmountMin).describe('`netAmount + taxAmount`, not re-derived from a price.')
})).optional().describe('One row per distinct VAT rate charged on this order, sorted ascending. Reconciles exactly: summed `netAmount` is `netTotal` + `shippingNetAmount`, summed `taxAmount` is `taxTotal`, and summed `grossAmount` is `totalPrice`. Absent on a pre-VAT order.'),
  ...
  "paymentMethod": zod.enum(['card', 'bank_transfer']).optional().describe('How an order is being paid for. A preference recorded at checkout, not a lock — a card payment still settles normally regardless of this value.'),
  "payBy": zod.iso.datetime({"offset":true}).optional(),
  "invoiceNumber": zod.string().optional(),
  "invoicePdfStatus": zod.enum(['pending', 'ready']).optional(),
  "transferInstructions": zod.strictObject({
  "beneficiary": zod.string(),
  "iban": zod.string(),
  "bic": zod.string().optional().describe('Absent when the deployment has not configured one.'),
  "reference": zod.string().describe('The ISO 11649 \"RF\" creditor reference minted for this order at checkout — what the customer writes into the transfer\'s description, so an admin can match the incoming payment back to it via `GET \/payments\/order-by-reference`. An order that predates this field falls back to its own raw id, which that same endpoint also accepts.')
}).optional(),
...
export const LoginTwoFactorBody = zod.strictObject({
  "challenge": zod.string().optional().describe('The challenge token from POST \/account\/login. Omit it for an OAuth-originated login — that challenge was never sent to the client, and this endpoint reads it from the httpOnly cookie the callback set instead.'),
...
export const SendTwoFactorCodeBody = zod.strictObject({
  "challenge": zod.string().optional().describe('The challenge token from POST \/account\/login. Omit it for an OAuth-originated login — see LoginTwoFactorRequest.challenge.'),
...
export const ListUsersQueryParams = zod.strictObject({
  ...
  "text": zod.string().min(1).optional(),
  "id": zod.array(zod.string().describe('Resource identifier')).min(1).max(listUsersQueryIdMax).optional(),
  "email": zod.email().optional(),
...
export const SearchUsersBody = zod.strictObject({
  ...
  "id": zod.array(zod.string().describe('Resource identifier')).min(1).max(searchUsersBodyIdMax).optional(),
  "email": zod.email().optional(),
  "username": zod.string().optional(),
  "active": zod.boolean().optional()
})
...
export const CreateFeedbackRequestHeader = zod.strictObject({
  "x-antibot-challenge-token": zod.string().optional().describe('The token the active human-challenge provider issued to the client — see `GET \/antibot\/config`. Absent when that provider is `none`.\n'),
  "Idempotency-Key": zod.string().min(1).max(createFeedbackRequestHeaderIdempotencyKeyMax).regex(createFeedbackRequestHeaderIdempotencyKeyRegExp).optional().describe('An opaque, client-generated value (a UUID by convention) that makes a retried write safe. Repeating this request with the SAME key and the SAME body replays the first response (`Idempotent-Replay: true`, no repeated write) instead of running it again; the same key with a DIFFERENT body answers 422; a key still being processed by another in-flight request answers 409. Omitting the header simply forgoes replay protection — the write still happens normally.\n')
})
...
export const ListProductsQueryParams = zod.strictObject({
  ...
  "text": zod.string().min(1).optional(),
  "id": zod.array(zod.string().describe('Resource identifier')).min(1).max(listProductsQueryIdMax).optional(),
...
export const ListProductsResponse = zod.strictObject({
  ...
  "id": zod.string().describe('Resource identifier'),
  "title": zod.string(),
  "price": zod.number().min(listProductsResponseDataItemsItemPriceMin).describe('Gross — what the customer pays, VAT included. Never net-of-tax: the invoice derives the net amount and the VAT amount FROM this, at whatever rate applies, rather than the other way around.'),
  "taxClass": zod.enum(['reduced', 'zero']).optional().describe('A category of goods taxed below the shop\'s standard VAT rate — books, food, medicine and similar, depending on the deployment\'s own jurisdiction. Absent means the standard rate.'),
...
export const CreateProductBody = zod.strictObject({
  "translations": zod.record(zod.string(), zod.strictObject({
  "title": zod.string(),
  "description": zod.string().optional()
}).nullable()).describe('One or more locales, keyed by BCP 47 tag. A key absent from this map leaves that locale untouched; a key mapped to an object upserts it; a key mapped to null deletes it — never omission or an empty object, which are both errors. The fallback locale (`NODE_FALLBACK_LOCALE`) MUST be present and non-null on create, and MUST NOT be null on update.'),
  "price": zod.number().min(createProductBodyPriceMin).describe('Gross — VAT included. Same convention as `Product.price`.'),
  "taxClass": zod.enum(['reduced', 'zero']).optional().describe('A category of goods taxed below the shop\'s standard VAT rate — books, food, medicine and similar, depending on the deployment\'s own jurisdiction. Absent means the standard rate.'),
  ...
  "weight": zod.number().min(createProductBodyWeightMin).optional().describe('Grams. Absent counts as 0 for shipping-method filtering.'),
  "imageUrl": zod.string().optional().describe('Absolute URL or server-relative upload path (e.g. `\/uploads\/abc.jpg`). `uri-reference`, not `uri`: an uploaded image is stored and returned as a path relative to the API host, which is not a valid absolute URI.'),
  "categories": zod.array(zod.string()).optional(),
  "tags": zod.array(zod.string()).optional()
})
...
export const CreateProductResponse = zod.strictObject({
  ...
  "id": zod.string().describe('Resource identifier'),
  "title": zod.string(),
  "price": zod.number().min(createProductResponseDataPriceMin).describe('Gross — what the customer pays, VAT included. Never net-of-tax: the invoice derives the net amount and the VAT amount FROM this, at whatever rate applies, rather than the other way around.'),
  "taxClass": zod.enum(['reduced', 'zero']).optional().describe('A category of goods taxed below the shop\'s standard VAT rate — books, food, medicine and similar, depending on the deployment\'s own jurisdiction. Absent means the standard rate.'),
...
export const GetProductByIdResponse = zod.strictObject({
  ...
  "id": zod.string().describe('Resource identifier'),
  "title": zod.string(),
  "price": zod.number().min(getProductByIdResponseDataPriceMin).describe('Gross — what the customer pays, VAT included. Never net-of-tax: the invoice derives the net amount and the VAT amount FROM this, at whatever rate applies, rather than the other way around.'),
  "taxClass": zod.enum(['reduced', 'zero']).optional().describe('A category of goods taxed below the shop\'s standard VAT rate — books, food, medicine and similar, depending on the deployment\'s own jurisdiction. Absent means the standard rate.'),
...
export const UpdateProductByIdBody = zod.strictObject({
  ...
  "title": zod.string(),
  "description": zod.string().optional()
}).nullable()).optional().describe('One or more locales, keyed by BCP 47 tag. A key absent from this map leaves that locale untouched; a key mapped to an object upserts it; a key mapped to null deletes it — never omission or an empty object, which are both errors. The fallback locale (`NODE_FALLBACK_LOCALE`) MUST be present and non-null on create, and MUST NOT be null on update.'),
  "price": zod.number().min(updateProductByIdBodyPriceMin).optional().describe('Gross — VAT included. Same convention as `Product.price`.'),
  "taxClass": zod.enum(['reduced', 'zero']).optional().describe('A category of goods taxed below the shop\'s standard VAT rate — books, food, medicine and similar, depending on the deployment\'s own jurisdiction. Absent means the standard rate.'),
  "active": zod.boolean().optional(),
  "requiresShipping": zod.boolean().optional(),
  "weight": zod.number().min(updateProductByIdBodyWeightMin).optional().describe('Grams. Absent counts as 0 for shipping-method filtering.'),
  "imageUrl": zod.string().optional().describe('Absolute URL or server-relative upload path (e.g. `\/uploads\/abc.jpg`). `uri-reference`, not `uri`: an uploaded image is stored and returned as a path relative to the API host, which is not a valid absolute URI.'),
  "categories": zod.array(zod.string()).optional(),
  "tags": zod.array(zod.string()).optional()
})
...
export const UpdateProductByIdResponse = zod.strictObject({
  ...
  "id": zod.string().describe('Resource identifier'),
  "title": zod.string(),
  "price": zod.number().min(updateProductByIdResponseDataPriceMin).describe('Gross — what the customer pays, VAT included. Never net-of-tax: the invoice derives the net amount and the VAT amount FROM this, at whatever rate applies, rather than the other way around.'),
  "taxClass": zod.enum(['reduced', 'zero']).optional().describe('A category of goods taxed below the shop\'s standard VAT rate — books, food, medicine and similar, depending on the deployment\'s own jurisdiction. Absent means the standard rate.'),
...
export const GetProductAdminResponse = zod.strictObject({
  ...
  "id": zod.string().describe('Resource identifier'),
  "title": zod.string(),
  "price": zod.number().min(getProductAdminResponseDataPriceMin).describe('Gross — VAT included. Same convention as `Product.price`.'),
  "taxClass": zod.enum(['reduced', 'zero']).optional().describe('A category of goods taxed below the shop\'s standard VAT rate — books, food, medicine and similar, depending on the deployment\'s own jurisdiction. Absent means the standard rate.'),
  ...
  "weight": zod.number().min(getProductAdminResponseDataWeightMin).optional().describe('Grams. Absent counts as 0 for shipping-method filtering — the editor warns when it\'s missing.'),
  "imageUrl": zod.string().optional().describe('Absolute URL or server-relative upload path (e.g. `\/uploads\/abc.jpg`). `uri-reference`, not `uri`: an uploaded image is stored and returned as a path relative to the API host, which is not a valid absolute URI.'),
  "thumbnailUrl": zod.string().optional().describe('Server-relative path to a small WebP derivative of `imageUrl`, produced by the image digest pipeline once an uploaded image has finished processing (see `docs\/tools\/image-processing.md`). Absent for a record whose image is a remote or default URL rather than an upload — there is nothing to derive a thumbnail from. Never accepted on a request body: the server is the only writer.'),
  "categories": zod.array(zod.string()).optional(),
  "tags": zod.array(zod.string()).optional(),
...
}).describe('A product with every language it has a row for, not just the caller\'s resolved one — what the editor\'s form needs to populate its tabs. `title`\/`description` stay the resolved (fallback-locale-derived) values; `translations` is the per-locale source.')
})
...
export const SearchProductsBody = zod.strictObject({
  ...
  "text": zod.string().min(1).optional().describe('Free-text search string'),
  "id": zod.array(zod.string().describe('Resource identifier')).min(1).max(searchProductsBodyIdMax).optional(),
...
export const SearchProductsResponse = zod.strictObject({
  ...
  "id": zod.string().describe('Resource identifier'),
  "title": zod.string(),
  "price": zod.number().min(searchProductsResponseDataItemsItemPriceMin).describe('Gross — what the customer pays, VAT included. Never net-of-tax: the invoice derives the net amount and the VAT amount FROM this, at whatever rate applies, rather than the other way around.'),
  "taxClass": zod.enum(['reduced', 'zero']).optional().describe('A category of goods taxed below the shop\'s standard VAT rate — books, food, medicine and similar, depending on the deployment\'s own jurisdiction. Absent means the standard rate.'),
...
export const CheckoutBody = zod.strictObject({
  ...
  "paymentMethod": zod.enum(['card', 'bank_transfer']).describe('How an order is being paid for. A preference recorded at checkout, not a lock — a card payment still settles normally regardless of this value.').default(checkoutBodyPaymentMethodDefault).describe('How the customer intends to pay (see `GET \/payments\/methods`). `card` holds stock for `NODE_RESERVATION_TTL_MINUTES`; `bank_transfer` holds it for `NODE_BANK_TRANSFER_HOLD_HOURS` instead, and the response carries `transferInstructions`. A method this deployment does not offer refuses the checkout with 409, `errors[].code` `CART_PAYMENT_METHOD_NOT_AVAILABLE`.')
})
...
export const checkoutResponseDataOrderItemsItemLocaleRegExp = new RegExp('^[a-z]{2}(-[A-Za-z0-9]+)*$');
...
export const CheckoutResponse = zod.strictObject({
  ...
  "id": zod.string().describe('Resource identifier'),
  "title": zod.string(),
  "price": zod.number().min(checkoutResponseDataOrderItemsItemProductPriceMin).describe('Gross — what the customer paid, VAT included, frozen at checkout. Same convention as `Product.price`.'),
  "description": zod.string().optional(),
  "active": zod.boolean().optional(),
  "requiresShipping": zod.boolean().default(checkoutResponseDataOrderItemsItemProductRequiresShippingDefault),
  "weight": zod.number().min(checkoutResponseDataOrderItemsItemProductWeightMin).optional(),
  "categories": zod.array(zod.string()).optional(),
  "tags": zod.array(zod.string()).optional(),
  "createdAt": zod.iso.datetime({"offset":true}).optional(),
  "updatedAt": zod.iso.datetime({"offset":true}).optional(),
  "deletedAt": zod.iso.datetime({"offset":true}).optional(),
  "taxRate": zod.number().min(checkoutResponseDataOrderItemsItemProductTaxRateMin).max(checkoutResponseDataOrderItemsItemProductTaxRateMax).optional().describe('The VAT rate this line was actually charged, as a decimal (0.22 for 22%). Frozen at checkout, same reasoning as `price`. Absent on an order placed before VAT existed.')
}),
  "quantity": zod.number().min(1),
  "locale": zod.string().regex(checkoutResponseDataOrderItemsItemLocaleRegExp).describe('BCP 47 language tag, e.g. `en` or `it`. Which tags a deployment actually supports is a runtime fact, not a contract one — ask `GET \/locales`.'),
  "current": zod.strictObject({
  "imageUrl": zod.string().describe('Absolute URL or server-relative upload path (e.g. `\/uploads\/abc.jpg`). `uri-reference`, not `uri`: an uploaded image is stored and returned as a path relative to the API host, which is not a valid absolute URI.'),
  "thumbnailUrl": zod.string().optional().describe('Server-relative path to a small WebP derivative of `imageUrl`, produced by the image digest pipeline once an uploaded image has finished processing (see `docs\/tools\/image-processing.md`). Absent for a record whose image is a remote or default URL rather than an upload — there is nothing to derive a thumbnail from. Never accepted on a request body: the server is the only writer.')
}).nullable().describe('The product\'s picture, resolved live — `null` when the catalogue product (`product.id`) has been hard-deleted. Never the terms of the sale, so it is never frozen; see `OrderLineCurrent`.'),
  "taxAmount": zod.number().min(checkoutResponseDataOrderItemsItemTaxAmountMin).optional().describe('VAT included in this line\'s total, at its own frozen `taxRate`.'),
  "netAmount": zod.number().min(checkoutResponseDataOrderItemsItemNetAmountMin).optional().describe('This line\'s total excluding VAT — `price × quantity` minus `taxAmount`.')
})),
  ...
  "netTotal": zod.number().min(checkoutResponseDataOrderNetTotalMin).optional().describe('Sum of every line\'s `netAmount` — the goods total excluding VAT, GOODS ONLY. Shipping\'s own net amount is `shippingNetAmount`, not folded in here. Absent on a pre-VAT order.'),
  "taxTotal": zod.number().min(checkoutResponseDataOrderTaxTotalMin).optional().describe('Every VAT collected on this order: the lines\' own `taxAmount`, plus the VAT on `shippingCost` — apportioned pro-rata across the lines by value and taxed at each line\'s own rate, since delivery is taxed as ancillary to what it delivers. Absent on a pre-VAT order.'),
  "shippingNetAmount": zod.number().min(checkoutResponseDataOrderShippingNetAmountMin).optional().describe('Shipping\'s own net amount, summed across every line it was apportioned onto — `netTotal` stays goods-only, this is the rest of the split. Absent on a pre-VAT order.'),
  "shippingTaxAmount": zod.number().min(checkoutResponseDataOrderShippingTaxAmountMin).optional().describe('Shipping\'s own tax amount, summed across every line it was apportioned onto — already folded into `taxTotal`, published separately so an invoice can print it as its own line. Absent on a pre-VAT order.'),
  "taxSummary": zod.array(zod.strictObject({
  "rate": zod.number().min(checkoutResponseDataOrderTaxSummaryItemRateMin).describe('The decimal rate this row is for (`0.22` for 22%) — never repeated across rows.'),
  "netAmount": zod.number().min(checkoutResponseDataOrderTaxSummaryItemNetAmountMin),
  "taxAmount": zod.number().min(checkoutResponseDataOrderTaxSummaryItemTaxAmountMin),
  "grossAmount": zod.number().min(checkoutResponseDataOrderTaxSummaryItemGrossAmountMin).describe('`netAmount + taxAmount`, not re-derived from a price.')
})).optional().describe('One row per distinct VAT rate charged on this order, sorted ascending. Reconciles exactly: summed `netAmount` is `netTotal` + `shippingNetAmount`, summed `taxAmount` is `taxTotal`, and summed `grossAmount` is `totalPrice`. Absent on a pre-VAT order.'),
  "notes": zod.string().optional().describe('Optional order notes'),
  "shippingMethod": zod.string().optional().describe('The shipping method\'s id as the checkout froze it (e.g. standard, express, pickup).'),
...
export const ListOrdersQueryParams = zod.strictObject({
  ...
  "id": zod.array(zod.string().describe('Resource identifier')).min(1).max(listOrdersQueryIdMax).optional(),
  "userId": zod.string().optional(),
  "productId": zod.string().optional(),
  "email": zod.email().optional(),
  "status": zod.enum(['pending', 'paid', 'processing', 'shipped', 'delivered', 'cancelled']).optional(),
  "paymentMethod": zod.enum(['card', 'bank_transfer']).optional().describe('Filter to orders placed with this method — the admin \"awaiting transfer\" view combines this with `status=pending`.'),
...
export const listOrdersResponseDataItemsItemItemsItemLocaleRegExp = new RegExp('^[a-z]{2}(-[A-Za-z0-9]+)*$');
...
export const ListOrdersResponse = zod.strictObject({
  ...
  "id": zod.string().describe('Resource identifier'),
  "title": zod.string(),
  "price": zod.number().min(listOrdersResponseDataItemsItemItemsItemProductPriceMin).describe('Gross — what the customer paid, VAT included, frozen at checkout. Same convention as `Product.price`.'),
  "description": zod.string().optional(),
  "active": zod.boolean().optional(),
  "requiresShipping": zod.boolean().default(listOrdersResponseDataItemsItemItemsItemProductRequiresShippingDefault),
  "weight": zod.number().min(listOrdersResponseDataItemsItemItemsItemProductWeightMin).optional(),
  "categories": zod.array(zod.string()).optional(),
  "tags": zod.array(zod.string()).optional(),
  "createdAt": zod.iso.datetime({"offset":true}).optional(),
  "updatedAt": zod.iso.datetime({"offset":true}).optional(),
  "deletedAt": zod.iso.datetime({"offset":true}).optional(),
  "taxRate": zod.number().min(listOrdersResponseDataItemsItemItemsItemProductTaxRateMin).max(listOrdersResponseDataItemsItemItemsItemProductTaxRateMax).optional().describe('The VAT rate this line was actually charged, as a decimal (0.22 for 22%). Frozen at checkout, same reasoning as `price`. Absent on an order placed before VAT existed.')
}),
  "quantity": zod.number().min(1),
  "locale": zod.string().regex(listOrdersResponseDataItemsItemItemsItemLocaleRegExp).describe('BCP 47 language tag, e.g. `en` or `it`. Which tags a deployment actually supports is a runtime fact, not a contract one — ask `GET \/locales`.'),
  "current": zod.strictObject({
  "imageUrl": zod.string().describe('Absolute URL or server-relative upload path (e.g. `\/uploads\/abc.jpg`). `uri-reference`, not `uri`: an uploaded image is stored and returned as a path relative to the API host, which is not a valid absolute URI.'),
  "thumbnailUrl": zod.string().optional().describe('Server-relative path to a small WebP derivative of `imageUrl`, produced by the image digest pipeline once an uploaded image has finished processing (see `docs\/tools\/image-processing.md`). Absent for a record whose image is a remote or default URL rather than an upload — there is nothing to derive a thumbnail from. Never accepted on a request body: the server is the only writer.')
}).nullable().describe('The product\'s picture, resolved live — `null` when the catalogue product (`product.id`) has been hard-deleted. Never the terms of the sale, so it is never frozen; see `OrderLineCurrent`.'),
  "taxAmount": zod.number().min(listOrdersResponseDataItemsItemItemsItemTaxAmountMin).optional().describe('VAT included in this line\'s total, at its own frozen `taxRate`.'),
  "netAmount": zod.number().min(listOrdersResponseDataItemsItemItemsItemNetAmountMin).optional().describe('This line\'s total excluding VAT — `price × quantity` minus `taxAmount`.')
})),
  ...
  "netTotal": zod.number().min(listOrdersResponseDataItemsItemNetTotalMin).optional().describe('Sum of every line\'s `netAmount` — the goods total excluding VAT, GOODS ONLY. Shipping\'s own net amount is `shippingNetAmount`, not folded in here. Absent on a pre-VAT order.'),
  "taxTotal": zod.number().min(listOrdersResponseDataItemsItemTaxTotalMin).optional().describe('Every VAT collected on this order: the lines\' own `taxAmount`, plus the VAT on `shippingCost` — apportioned pro-rata across the lines by value and taxed at each line\'s own rate, since delivery is taxed as ancillary to what it delivers. Absent on a pre-VAT order.'),
  "shippingNetAmount": zod.number().min(listOrdersResponseDataItemsItemShippingNetAmountMin).optional().describe('Shipping\'s own net amount, summed across every line it was apportioned onto — `netTotal` stays goods-only, this is the rest of the split. Absent on a pre-VAT order.'),
  "shippingTaxAmount": zod.number().min(listOrdersResponseDataItemsItemShippingTaxAmountMin).optional().describe('Shipping\'s own tax amount, summed across every line it was apportioned onto — already folded into `taxTotal`, published separately so an invoice can print it as its own line. Absent on a pre-VAT order.'),
  "taxSummary": zod.array(zod.strictObject({
  "rate": zod.number().min(listOrdersResponseDataItemsItemTaxSummaryItemRateMin).describe('The decimal rate this row is for (`0.22` for 22%) — never repeated across rows.'),
  "netAmount": zod.number().min(listOrdersResponseDataItemsItemTaxSummaryItemNetAmountMin),
  "taxAmount": zod.number().min(listOrdersResponseDataItemsItemTaxSummaryItemTaxAmountMin),
  "grossAmount": zod.number().min(listOrdersResponseDataItemsItemTaxSummaryItemGrossAmountMin).describe('`netAmount + taxAmount`, not re-derived from a price.')
})).optional().describe('One row per distinct VAT rate charged on this order, sorted ascending. Reconciles exactly: summed `netAmount` is `netTotal` + `shippingNetAmount`, summed `taxAmount` is `taxTotal`, and summed `grossAmount` is `totalPrice`. Absent on a pre-VAT order.'),
  "notes": zod.string().optional().describe('Optional order notes'),
  "shippingMethod": zod.string().optional().describe('The shipping method\'s id as the checkout froze it (e.g. standard, express, pickup).'),
...
export const CreateOrderHeader = zod.strictObject({
  "Idempotency-Key": zod.string().min(1).max(createOrderHeaderIdempotencyKeyMax).regex(createOrderHeaderIdempotencyKeyRegExp).optional().describe('An opaque, client-generated value (a UUID by convention) that makes a retried write safe. Repeating this request with the SAME key and the SAME body replays the first response (`Idempotent-Replay: true`, no repeated write) instead of running it again; the same key with a DIFFERENT body answers 422; a key still being processed by another in-flight request answers 409. Omitting the header simply forgoes replay protection — the write still happens normally.\n')
})
...
export const createOrderResponseDataItemsItemLocaleRegExp = new RegExp('^[a-z]{2}(-[A-Za-z0-9]+)*$');
...
export const CreateOrderResponse = zod.strictObject({
  ...
  "id": zod.string().describe('Resource identifier'),
  "title": zod.string(),
  "price": zod.number().min(createOrderResponseDataItemsItemProductPriceMin).describe('Gross — what the customer paid, VAT included, frozen at checkout. Same convention as `Product.price`.'),
  "description": zod.string().optional(),
  "active": zod.boolean().optional(),
  "requiresShipping": zod.boolean().default(createOrderResponseDataItemsItemProductRequiresShippingDefault),
  "weight": zod.number().min(createOrderResponseDataItemsItemProductWeightMin).optional(),
  "categories": zod.array(zod.string()).optional(),
  "tags": zod.array(zod.string()).optional(),
  "createdAt": zod.iso.datetime({"offset":true}).optional(),
  "updatedAt": zod.iso.datetime({"offset":true}).optional(),
  "deletedAt": zod.iso.datetime({"offset":true}).optional(),
  "taxRate": zod.number().min(createOrderResponseDataItemsItemProductTaxRateMin).max(createOrderResponseDataItemsItemProductTaxRateMax).optional().describe('The VAT rate this line was actually charged, as a decimal (0.22 for 22%). Frozen at checkout, same reasoning as `price`. Absent on an order placed before VAT existed.')
}),
  "quantity": zod.number().min(1),
  "locale": zod.string().regex(createOrderResponseDataItemsItemLocaleRegExp).describe('BCP 47 language tag, e.g. `en` or `it`. Which tags a deployment actually supports is a runtime fact, not a contract one — ask `GET \/locales`.'),
  "current": zod.strictObject({
  "imageUrl": zod.string().describe('Absolute URL or server-relative upload path (e.g. `\/uploads\/abc.jpg`). `uri-reference`, not `uri`: an uploaded image is stored and returned as a path relative to the API host, which is not a valid absolute URI.'),
  "thumbnailUrl": zod.string().optional().describe('Server-relative path to a small WebP derivative of `imageUrl`, produced by the image digest pipeline once an uploaded image has finished processing (see `docs\/tools\/image-processing.md`). Absent for a record whose image is a remote or default URL rather than an upload — there is nothing to derive a thumbnail from. Never accepted on a request body: the server is the only writer.')
}).nullable().describe('The product\'s picture, resolved live — `null` when the catalogue product (`product.id`) has been hard-deleted. Never the terms of the sale, so it is never frozen; see `OrderLineCurrent`.'),
  "taxAmount": zod.number().min(createOrderResponseDataItemsItemTaxAmountMin).optional().describe('VAT included in this line\'s total, at its own frozen `taxRate`.'),
  "netAmount": zod.number().min(createOrderResponseDataItemsItemNetAmountMin).optional().describe('This line\'s total excluding VAT — `price × quantity` minus `taxAmount`.')
})),
  ...
  "netTotal": zod.number().min(createOrderResponseDataNetTotalMin).optional().describe('Sum of every line\'s `netAmount` — the goods total excluding VAT, GOODS ONLY. Shipping\'s own net amount is `shippingNetAmount`, not folded in here. Absent on a pre-VAT order.'),
  "taxTotal": zod.number().min(createOrderResponseDataTaxTotalMin).optional().describe('Every VAT collected on this order: the lines\' own `taxAmount`, plus the VAT on `shippingCost` — apportioned pro-rata across the lines by value and taxed at each line\'s own rate, since delivery is taxed as ancillary to what it delivers. Absent on a pre-VAT order.'),
  "shippingNetAmount": zod.number().min(createOrderResponseDataShippingNetAmountMin).optional().describe('Shipping\'s own net amount, summed across every line it was apportioned onto — `netTotal` stays goods-only, this is the rest of the split. Absent on a pre-VAT order.'),
  "shippingTaxAmount": zod.number().min(createOrderResponseDataShippingTaxAmountMin).optional().describe('Shipping\'s own tax amount, summed across every line it was apportioned onto — already folded into `taxTotal`, published separately so an invoice can print it as its own line. Absent on a pre-VAT order.'),
  "taxSummary": zod.array(zod.strictObject({
  "rate": zod.number().min(createOrderResponseDataTaxSummaryItemRateMin).describe('The decimal rate this row is for (`0.22` for 22%) — never repeated across rows.'),
  ...
  "grossAmount": zod.number().min(createOrderResponseDataTaxSummaryItemGrossAmountMin).describe('`netAmount + taxAmount`, not re-derived from a price.')
})).optional().describe('One row per distinct VAT rate charged on this order, sorted ascending. Reconciles exactly: summed `netAmount` is `netTotal` + `shippingNetAmount`, summed `taxAmount` is `taxTotal`, and summed `grossAmount` is `totalPrice`. Absent on a pre-VAT order.'),
  "notes": zod.string().optional().describe('Optional order notes'),
  "shippingMethod": zod.string().optional().describe('The shipping method\'s id as the checkout froze it (e.g. standard, express, pickup).'),
...
export const updateOrderResponseDataItemsItemProductWeightMin = 0;
...
export const updateOrderResponseDataItemsItemLocaleRegExp = new RegExp('^[a-z]{2}(-[A-Za-z0-9]+)*$');
...
export const updateOrderResponseDataShippingNetAmountMin = 0;

export const updateOrderResponseDataShippingTaxAmountMin = 0;

export const updateOrderResponseDataTaxSummaryItemRateMin = 0;

export const updateOrderResponseDataTaxSummaryItemNetAmountMin = 0;

export const updateOrderResponseDataTaxSummaryItemTaxAmountMin = 0;

export const updateOrderResponseDataTaxSummaryItemGrossAmountMin = 0;
...
export const UpdateOrderResponse = zod.strictObject({
  ...
  "id": zod.string().describe('Resource identifier'),
  "title": zod.string(),
  "price": zod.number().min(updateOrderResponseDataItemsItemProductPriceMin).describe('Gross — what the customer paid, VAT included, frozen at checkout. Same convention as `Product.price`.'),
  "description": zod.string().optional(),
  "active": zod.boolean().optional(),
  "requiresShipping": zod.boolean().default(updateOrderResponseDataItemsItemProductRequiresShippingDefault),
  "weight": zod.number().min(updateOrderResponseDataItemsItemProductWeightMin).optional(),
  "categories": zod.array(zod.string()).optional(),
  "tags": zod.array(zod.string()).optional(),
  "createdAt": zod.iso.datetime({"offset":true}).optional(),
  "updatedAt": zod.iso.datetime({"offset":true}).optional(),
  "deletedAt": zod.iso.datetime({"offset":true}).optional(),
  "taxRate": zod.number().min(updateOrderResponseDataItemsItemProductTaxRateMin).max(updateOrderResponseDataItemsItemProductTaxRateMax).optional().describe('The VAT rate this line was actually charged, as a decimal (0.22 for 22%). Frozen at checkout, same reasoning as `price`. Absent on an order placed before VAT existed.')
}),
  "quantity": zod.number().min(1),
  "locale": zod.string().regex(updateOrderResponseDataItemsItemLocaleRegExp).describe('BCP 47 language tag, e.g. `en` or `it`. Which tags a deployment actually supports is a runtime fact, not a contract one — ask `GET \/locales`.'),
  "current": zod.strictObject({
  "imageUrl": zod.string().describe('Absolute URL or server-relative upload path (e.g. `\/uploads\/abc.jpg`). `uri-reference`, not `uri`: an uploaded image is stored and returned as a path relative to the API host, which is not a valid absolute URI.'),
  "thumbnailUrl": zod.string().optional().describe('Server-relative path to a small WebP derivative of `imageUrl`, produced by the image digest pipeline once an uploaded image has finished processing (see `docs\/tools\/image-processing.md`). Absent for a record whose image is a remote or default URL rather than an upload — there is nothing to derive a thumbnail from. Never accepted on a request body: the server is the only writer.')
}).nullable().describe('The product\'s picture, resolved live — `null` when the catalogue product (`product.id`) has been hard-deleted. Never the terms of the sale, so it is never frozen; see `OrderLineCurrent`.'),
  "taxAmount": zod.number().min(updateOrderResponseDataItemsItemTaxAmountMin).optional().describe('VAT included in this line\'s total, at its own frozen `taxRate`.'),
  "netAmount": zod.number().min(updateOrderResponseDataItemsItemNetAmountMin).optional().describe('This line\'s total excluding VAT — `price × quantity` minus `taxAmount`.')
})),
  ...
  "netTotal": zod.number().min(updateOrderResponseDataNetTotalMin).optional().describe('Sum of every line\'s `netAmount` — the goods total excluding VAT, GOODS ONLY. Shipping\'s own net amount is `shippingNetAmount`, not folded in here. Absent on a pre-VAT order.'),
  "taxTotal": zod.number().min(updateOrderResponseDataTaxTotalMin).optional().describe('Every VAT collected on this order: the lines\' own `taxAmount`, plus the VAT on `shippingCost` — apportioned pro-rata across the lines by value and taxed at each line\'s own rate, since delivery is taxed as ancillary to what it delivers. Absent on a pre-VAT order.'),
  "shippingNetAmount": zod.number().min(updateOrderResponseDataShippingNetAmountMin).optional().describe('Shipping\'s own net amount, summed across every line it was apportioned onto — `netTotal` stays goods-only, this is the rest of the split. Absent on a pre-VAT order.'),
  "shippingTaxAmount": zod.number().min(updateOrderResponseDataShippingTaxAmountMin).optional().describe('Shipping\'s own tax amount, summed across every line it was apportioned onto — already folded into `taxTotal`, published separately so an invoice can print it as its own line. Absent on a pre-VAT order.'),
  "taxSummary": zod.array(zod.strictObject({
  "rate": zod.number().min(updateOrderResponseDataTaxSummaryItemRateMin).describe('The decimal rate this row is for (`0.22` for 22%) — never repeated across rows.'),
  "netAmount": zod.number().min(updateOrderResponseDataTaxSummaryItemNetAmountMin),
  "taxAmount": zod.number().min(updateOrderResponseDataTaxSummaryItemTaxAmountMin),
  "grossAmount": zod.number().min(updateOrderResponseDataTaxSummaryItemGrossAmountMin).describe('`netAmount + taxAmount`, not re-derived from a price.')
})).optional().describe('One row per distinct VAT rate charged on this order, sorted ascending. Reconciles exactly: summed `netAmount` is `netTotal` + `shippingNetAmount`, summed `taxAmount` is `taxTotal`, and summed `grossAmount` is `totalPrice`. Absent on a pre-VAT order.'),
  "notes": zod.string().optional().describe('Optional order notes'),
  "shippingMethod": zod.string().optional().describe('The shipping method\'s id as the checkout froze it (e.g. standard, express, pickup).'),
...
export const SearchOrdersBody = zod.strictObject({
  ...
  "id": zod.array(zod.string().describe('Resource identifier')).min(1).max(searchOrdersBodyIdMax).optional(),
  "userId": zod.string().optional().describe('Resource identifier'),
  "productId": zod.string().optional().describe('Resource identifier'),
  "email": zod.email().optional(),
...
export const searchOrdersResponseDataItemsItemItemsItemLocaleRegExp = new RegExp('^[a-z]{2}(-[A-Za-z0-9]+)*$');
...
export const SearchOrdersResponse = zod.strictObject({
  ...
  "id": zod.string().describe('Resource identifier'),
  "title": zod.string(),
  "price": zod.number().min(searchOrdersResponseDataItemsItemItemsItemProductPriceMin).describe('Gross — what the customer paid, VAT included, frozen at checkout. Same convention as `Product.price`.'),
  "description": zod.string().optional(),
  "active": zod.boolean().optional(),
  "requiresShipping": zod.boolean().default(searchOrdersResponseDataItemsItemItemsItemProductRequiresShippingDefault),
  "weight": zod.number().min(searchOrdersResponseDataItemsItemItemsItemProductWeightMin).optional(),
  "categories": zod.array(zod.string()).optional(),
  "tags": zod.array(zod.string()).optional(),
  "createdAt": zod.iso.datetime({"offset":true}).optional(),
  "updatedAt": zod.iso.datetime({"offset":true}).optional(),
  "deletedAt": zod.iso.datetime({"offset":true}).optional(),
  "taxRate": zod.number().min(searchOrdersResponseDataItemsItemItemsItemProductTaxRateMin).max(searchOrdersResponseDataItemsItemItemsItemProductTaxRateMax).optional().describe('The VAT rate this line was actually charged, as a decimal (0.22 for 22%). Frozen at checkout, same reasoning as `price`. Absent on an order placed before VAT existed.')
}),
  "quantity": zod.number().min(1),
  "locale": zod.string().regex(searchOrdersResponseDataItemsItemItemsItemLocaleRegExp).describe('BCP 47 language tag, e.g. `en` or `it`. Which tags a deployment actually supports is a runtime fact, not a contract one — ask `GET \/locales`.'),
  "current": zod.strictObject({
  "imageUrl": zod.string().describe('Absolute URL or server-relative upload path (e.g. `\/uploads\/abc.jpg`). `uri-reference`, not `uri`: an uploaded image is stored and returned as a path relative to the API host, which is not a valid absolute URI.'),
  "thumbnailUrl": zod.string().optional().describe('Server-relative path to a small WebP derivative of `imageUrl`, produced by the image digest pipeline once an uploaded image has finished processing (see `docs\/tools\/image-processing.md`). Absent for a record whose image is a remote or default URL rather than an upload — there is nothing to derive a thumbnail from. Never accepted on a request body: the server is the only writer.')
}).nullable().describe('The product\'s picture, resolved live — `null` when the catalogue product (`product.id`) has been hard-deleted. Never the terms of the sale, so it is never frozen; see `OrderLineCurrent`.'),
  "taxAmount": zod.number().min(searchOrdersResponseDataItemsItemItemsItemTaxAmountMin).optional().describe('VAT included in this line\'s total, at its own frozen `taxRate`.'),
  "netAmount": zod.number().min(searchOrdersResponseDataItemsItemItemsItemNetAmountMin).optional().describe('This line\'s total excluding VAT — `price × quantity` minus `taxAmount`.')
})),
  ...
  "netTotal": zod.number().min(searchOrdersResponseDataItemsItemNetTotalMin).optional().describe('Sum of every line\'s `netAmount` — the goods total excluding VAT, GOODS ONLY. Shipping\'s own net amount is `shippingNetAmount`, not folded in here. Absent on a pre-VAT order.'),
  "taxTotal": zod.number().min(searchOrdersResponseDataItemsItemTaxTotalMin).optional().describe('Every VAT collected on this order: the lines\' own `taxAmount`, plus the VAT on `shippingCost` — apportioned pro-rata across the lines by value and taxed at each line\'s own rate, since delivery is taxed as ancillary to what it delivers. Absent on a pre-VAT order.'),
  "shippingNetAmount": zod.number().min(searchOrdersResponseDataItemsItemShippingNetAmountMin).optional().describe('Shipping\'s own net amount, summed across every line it was apportioned onto — `netTotal` stays goods-only, this is the rest of the split. Absent on a pre-VAT order.'),
  "shippingTaxAmount": zod.number().min(searchOrdersResponseDataItemsItemShippingTaxAmountMin).optional().describe('Shipping\'s own tax amount, summed across every line it was apportioned onto — already folded into `taxTotal`, published separately so an invoice can print it as its own line. Absent on a pre-VAT order.'),
  "taxSummary": zod.array(zod.strictObject({
  "rate": zod.number().min(searchOrdersResponseDataItemsItemTaxSummaryItemRateMin).describe('The decimal rate this row is for (`0.22` for 22%) — never repeated across rows.'),
  "netAmount": zod.number().min(searchOrdersResponseDataItemsItemTaxSummaryItemNetAmountMin),
  "taxAmount": zod.number().min(searchOrdersResponseDataItemsItemTaxSummaryItemTaxAmountMin),
  "grossAmount": zod.number().min(searchOrdersResponseDataItemsItemTaxSummaryItemGrossAmountMin).describe('`netAmount + taxAmount`, not re-derived from a price.')
})).optional().describe('One row per distinct VAT rate charged on this order, sorted ascending. Reconciles exactly: summed `netAmount` is `netTotal` + `shippingNetAmount`, summed `taxAmount` is `taxTotal`, and summed `grossAmount` is `totalPrice`. Absent on a pre-VAT order.'),
  "notes": zod.string().optional().describe('Optional order notes'),
  "shippingMethod": zod.string().optional().describe('The shipping method\'s id as the checkout froze it (e.g. standard, express, pickup).'),
...
export const getOrderByIdResponseDataItemsItemLocaleRegExp = new RegExp('^[a-z]{2}(-[A-Za-z0-9]+)*$');
...
export const GetOrderByIdResponse = zod.strictObject({
  ...
  "id": zod.string().describe('Resource identifier'),
  "title": zod.string(),
  "price": zod.number().min(getOrderByIdResponseDataItemsItemProductPriceMin).describe('Gross — what the customer paid, VAT included, frozen at checkout. Same convention as `Product.price`.'),
  "description": zod.string().optional(),
  "active": zod.boolean().optional(),
  "requiresShipping": zod.boolean().default(getOrderByIdResponseDataItemsItemProductRequiresShippingDefault),
  "weight": zod.number().min(getOrderByIdResponseDataItemsItemProductWeightMin).optional(),
  "categories": zod.array(zod.string()).optional(),
  "tags": zod.array(zod.string()).optional(),
  "createdAt": zod.iso.datetime({"offset":true}).optional(),
  "updatedAt": zod.iso.datetime({"offset":true}).optional(),
  "deletedAt": zod.iso.datetime({"offset":true}).optional(),
  "taxRate": zod.number().min(getOrderByIdResponseDataItemsItemProductTaxRateMin).max(getOrderByIdResponseDataItemsItemProductTaxRateMax).optional().describe('The VAT rate this line was actually charged, as a decimal (0.22 for 22%). Frozen at checkout, same reasoning as `price`. Absent on an order placed before VAT existed.')
}),
  "quantity": zod.number().min(1),
  "locale": zod.string().regex(getOrderByIdResponseDataItemsItemLocaleRegExp).describe('BCP 47 language tag, e.g. `en` or `it`. Which tags a deployment actually supports is a runtime fact, not a contract one — ask `GET \/locales`.'),
  "current": zod.strictObject({
  "imageUrl": zod.string().describe('Absolute URL or server-relative upload path (e.g. `\/uploads\/abc.jpg`). `uri-reference`, not `uri`: an uploaded image is stored and returned as a path relative to the API host, which is not a valid absolute URI.'),
  "thumbnailUrl": zod.string().optional().describe('Server-relative path to a small WebP derivative of `imageUrl`, produced by the image digest pipeline once an uploaded image has finished processing (see `docs\/tools\/image-processing.md`). Absent for a record whose image is a remote or default URL rather than an upload — there is nothing to derive a thumbnail from. Never accepted on a request body: the server is the only writer.')
}).nullable().describe('The product\'s picture, resolved live — `null` when the catalogue product (`product.id`) has been hard-deleted. Never the terms of the sale, so it is never frozen; see `OrderLineCurrent`.'),
  "taxAmount": zod.number().min(getOrderByIdResponseDataItemsItemTaxAmountMin).optional().describe('VAT included in this line\'s total, at its own frozen `taxRate`.'),
  "netAmount": zod.number().min(getOrderByIdResponseDataItemsItemNetAmountMin).optional().describe('This line\'s total excluding VAT — `price × quantity` minus `taxAmount`.')
})),
  ...
  "netTotal": zod.number().min(getOrderByIdResponseDataNetTotalMin).optional().describe('Sum of every line\'s `netAmount` — the goods total excluding VAT, GOODS ONLY. Shipping\'s own net amount is `shippingNetAmount`, not folded in here. Absent on a pre-VAT order.'),
  "taxTotal": zod.number().min(getOrderByIdResponseDataTaxTotalMin).optional().describe('Every VAT collected on this order: the lines\' own `taxAmount`, plus the VAT on `shippingCost` — apportioned pro-rata across the lines by value and taxed at each line\'s own rate, since delivery is taxed as ancillary to what it delivers. Absent on a pre-VAT order.'),
  "shippingNetAmount": zod.number().min(getOrderByIdResponseDataShippingNetAmountMin).optional().describe('Shipping\'s own net amount, summed across every line it was apportioned onto — `netTotal` stays goods-only, this is the rest of the split. Absent on a pre-VAT order.'),
  "shippingTaxAmount": zod.number().min(getOrderByIdResponseDataShippingTaxAmountMin).optional().describe('Shipping\'s own tax amount, summed across every line it was apportioned onto — already folded into `taxTotal`, published separately so an invoice can print it as its own line. Absent on a pre-VAT order.'),
  "taxSummary": zod.array(zod.strictObject({
  "rate": zod.number().min(getOrderByIdResponseDataTaxSummaryItemRateMin).describe('The decimal rate this row is for (`0.22` for 22%) — never repeated across rows.'),
  ...
  "grossAmount": zod.number().min(getOrderByIdResponseDataTaxSummaryItemGrossAmountMin).describe('`netAmount + taxAmount`, not re-derived from a price.')
})).optional().describe('One row per distinct VAT rate charged on this order, sorted ascending. Reconciles exactly: summed `netAmount` is `netTotal` + `shippingNetAmount`, summed `taxAmount` is `taxTotal`, and summed `grossAmount` is `totalPrice`. Absent on a pre-VAT order.'),
  "notes": zod.string().optional().describe('Optional order notes'),
  "shippingMethod": zod.string().optional().describe('The shipping method\'s id as the checkout froze it (e.g. standard, express, pickup).'),
...
export const updateOrderByIdResponseDataItemsItemLocaleRegExp = new RegExp('^[a-z]{2}(-[A-Za-z0-9]+)*$');
...
export const UpdateOrderByIdResponse = zod.strictObject({
  ...
  "id": zod.string().describe('Resource identifier'),
  "title": zod.string(),
  "price": zod.number().min(updateOrderByIdResponseDataItemsItemProductPriceMin).describe('Gross — what the customer paid, VAT included, frozen at checkout. Same convention as `Product.price`.'),
  "description": zod.string().optional(),
  "active": zod.boolean().optional(),
  "requiresShipping": zod.boolean().default(updateOrderByIdResponseDataItemsItemProductRequiresShippingDefault),
  "weight": zod.number().min(updateOrderByIdResponseDataItemsItemProductWeightMin).optional(),
  "categories": zod.array(zod.string()).optional(),
  "tags": zod.array(zod.string()).optional(),
  "createdAt": zod.iso.datetime({"offset":true}).optional(),
  "updatedAt": zod.iso.datetime({"offset":true}).optional(),
  "deletedAt": zod.iso.datetime({"offset":true}).optional(),
  "taxRate": zod.number().min(updateOrderByIdResponseDataItemsItemProductTaxRateMin).max(updateOrderByIdResponseDataItemsItemProductTaxRateMax).optional().describe('The VAT rate this line was actually charged, as a decimal (0.22 for 22%). Frozen at checkout, same reasoning as `price`. Absent on an order placed before VAT existed.')
}),
  "quantity": zod.number().min(1),
  "locale": zod.string().regex(updateOrderByIdResponseDataItemsItemLocaleRegExp).describe('BCP 47 language tag, e.g. `en` or `it`. Which tags a deployment actually supports is a runtime fact, not a contract one — ask `GET \/locales`.'),
  "current": zod.strictObject({
  "imageUrl": zod.string().describe('Absolute URL or server-relative upload path (e.g. `\/uploads\/abc.jpg`). `uri-reference`, not `uri`: an uploaded image is stored and returned as a path relative to the API host, which is not a valid absolute URI.'),
  "thumbnailUrl": zod.string().optional().describe('Server-relative path to a small WebP derivative of `imageUrl`, produced by the image digest pipeline once an uploaded image has finished processing (see `docs\/tools\/image-processing.md`). Absent for a record whose image is a remote or default URL rather than an upload — there is nothing to derive a thumbnail from. Never accepted on a request body: the server is the only writer.')
}).nullable().describe('The product\'s picture, resolved live — `null` when the catalogue product (`product.id`) has been hard-deleted. Never the terms of the sale, so it is never frozen; see `OrderLineCurrent`.'),
  "taxAmount": zod.number().min(updateOrderByIdResponseDataItemsItemTaxAmountMin).optional().describe('VAT included in this line\'s total, at its own frozen `taxRate`.'),
  "netAmount": zod.number().min(updateOrderByIdResponseDataItemsItemNetAmountMin).optional().describe('This line\'s total excluding VAT — `price × quantity` minus `taxAmount`.')
})),
  ...
  "netTotal": zod.number().min(updateOrderByIdResponseDataNetTotalMin).optional().describe('Sum of every line\'s `netAmount` — the goods total excluding VAT, GOODS ONLY. Shipping\'s own net amount is `shippingNetAmount`, not folded in here. Absent on a pre-VAT order.'),
  "taxTotal": zod.number().min(updateOrderByIdResponseDataTaxTotalMin).optional().describe('Every VAT collected on this order: the lines\' own `taxAmount`, plus the VAT on `shippingCost` — apportioned pro-rata across the lines by value and taxed at each line\'s own rate, since delivery is taxed as ancillary to what it delivers. Absent on a pre-VAT order.'),
  "shippingNetAmount": zod.number().min(updateOrderByIdResponseDataShippingNetAmountMin).optional().describe('Shipping\'s own net amount, summed across every line it was apportioned onto — `netTotal` stays goods-only, this is the rest of the split. Absent on a pre-VAT order.'),
  "shippingTaxAmount": zod.number().min(updateOrderByIdResponseDataShippingTaxAmountMin).optional().describe('Shipping\'s own tax amount, summed across every line it was apportioned onto — already folded into `taxTotal`, published separately so an invoice can print it as its own line. Absent on a pre-VAT order.'),
  "taxSummary": zod.array(zod.strictObject({
  "rate": zod.number().min(updateOrderByIdResponseDataTaxSummaryItemRateMin).describe('The decimal rate this row is for (`0.22` for 22%) — never repeated across rows.'),
  ...
  "grossAmount": zod.number().min(updateOrderByIdResponseDataTaxSummaryItemGrossAmountMin).describe('`netAmount + taxAmount`, not re-derived from a price.')
})).optional().describe('One row per distinct VAT rate charged on this order, sorted ascending. Reconciles exactly: summed `netAmount` is `netTotal` + `shippingNetAmount`, summed `taxAmount` is `taxTotal`, and summed `grossAmount` is `totalPrice`. Absent on a pre-VAT order.'),
  "notes": zod.string().optional().describe('Optional order notes'),
  "shippingMethod": zod.string().optional().describe('The shipping method\'s id as the checkout froze it (e.g. standard, express, pickup).'),
...
export const cancelOrderByIdResponseDataItemsItemLocaleRegExp = new RegExp('^[a-z]{2}(-[A-Za-z0-9]+)*$');
...
export const CancelOrderByIdResponse = zod.strictObject({
  ...
  "id": zod.string().describe('Resource identifier'),
  "title": zod.string(),
  "price": zod.number().min(cancelOrderByIdResponseDataItemsItemProductPriceMin).describe('Gross — what the customer paid, VAT included, frozen at checkout. Same convention as `Product.price`.'),
  "description": zod.string().optional(),
  "active": zod.boolean().optional(),
  "requiresShipping": zod.boolean().default(cancelOrderByIdResponseDataItemsItemProductRequiresShippingDefault),
  "weight": zod.number().min(cancelOrderByIdResponseDataItemsItemProductWeightMin).optional(),
  "categories": zod.array(zod.string()).optional(),
  "tags": zod.array(zod.string()).optional(),
  "createdAt": zod.iso.datetime({"offset":true}).optional(),
  "updatedAt": zod.iso.datetime({"offset":true}).optional(),
  "deletedAt": zod.iso.datetime({"offset":true}).optional(),
  "taxRate": zod.number().min(cancelOrderByIdResponseDataItemsItemProductTaxRateMin).max(cancelOrderByIdResponseDataItemsItemProductTaxRateMax).optional().describe('The VAT rate this line was actually charged, as a decimal (0.22 for 22%). Frozen at checkout, same reasoning as `price`. Absent on an order placed before VAT existed.')
}),
  "quantity": zod.number().min(1),
  "locale": zod.string().regex(cancelOrderByIdResponseDataItemsItemLocaleRegExp).describe('BCP 47 language tag, e.g. `en` or `it`. Which tags a deployment actually supports is a runtime fact, not a contract one — ask `GET \/locales`.'),
  "current": zod.strictObject({
  "imageUrl": zod.string().describe('Absolute URL or server-relative upload path (e.g. `\/uploads\/abc.jpg`). `uri-reference`, not `uri`: an uploaded image is stored and returned as a path relative to the API host, which is not a valid absolute URI.'),
  "thumbnailUrl": zod.string().optional().describe('Server-relative path to a small WebP derivative of `imageUrl`, produced by the image digest pipeline once an uploaded image has finished processing (see `docs\/tools\/image-processing.md`). Absent for a record whose image is a remote or default URL rather than an upload — there is nothing to derive a thumbnail from. Never accepted on a request body: the server is the only writer.')
}).nullable().describe('The product\'s picture, resolved live — `null` when the catalogue product (`product.id`) has been hard-deleted. Never the terms of the sale, so it is never frozen; see `OrderLineCurrent`.'),
  "taxAmount": zod.number().min(cancelOrderByIdResponseDataItemsItemTaxAmountMin).optional().describe('VAT included in this line\'s total, at its own frozen `taxRate`.'),
  "netAmount": zod.number().min(cancelOrderByIdResponseDataItemsItemNetAmountMin).optional().describe('This line\'s total excluding VAT — `price × quantity` minus `taxAmount`.')
})),
  ...
  "netTotal": zod.number().min(cancelOrderByIdResponseDataNetTotalMin).optional().describe('Sum of every line\'s `netAmount` — the goods total excluding VAT, GOODS ONLY. Shipping\'s own net amount is `shippingNetAmount`, not folded in here. Absent on a pre-VAT order.'),
  "taxTotal": zod.number().min(cancelOrderByIdResponseDataTaxTotalMin).optional().describe('Every VAT collected on this order: the lines\' own `taxAmount`, plus the VAT on `shippingCost` — apportioned pro-rata across the lines by value and taxed at each line\'s own rate, since delivery is taxed as ancillary to what it delivers. Absent on a pre-VAT order.'),
  "shippingNetAmount": zod.number().min(cancelOrderByIdResponseDataShippingNetAmountMin).optional().describe('Shipping\'s own net amount, summed across every line it was apportioned onto — `netTotal` stays goods-only, this is the rest of the split. Absent on a pre-VAT order.'),
  "shippingTaxAmount": zod.number().min(cancelOrderByIdResponseDataShippingTaxAmountMin).optional().describe('Shipping\'s own tax amount, summed across every line it was apportioned onto — already folded into `taxTotal`, published separately so an invoice can print it as its own line. Absent on a pre-VAT order.'),
  "taxSummary": zod.array(zod.strictObject({
  "rate": zod.number().min(cancelOrderByIdResponseDataTaxSummaryItemRateMin).describe('The decimal rate this row is for (`0.22` for 22%) — never repeated across rows.'),
  ...
  "grossAmount": zod.number().min(cancelOrderByIdResponseDataTaxSummaryItemGrossAmountMin).describe('`netAmount + taxAmount`, not re-derived from a price.')
})).optional().describe('One row per distinct VAT rate charged on this order, sorted ascending. Reconciles exactly: summed `netAmount` is `netTotal` + `shippingNetAmount`, summed `taxAmount` is `taxTotal`, and summed `grossAmount` is `totalPrice`. Absent on a pre-VAT order.'),
  "notes": zod.string().optional().describe('Optional order notes'),
  "shippingMethod": zod.string().optional().describe('The shipping method\'s id as the checkout froze it (e.g. standard, express, pickup).'),
...
export const OverrideOrderStatusBody = zod.strictObject({
  "to": zod.enum(['pending', 'paid', 'processing', 'shipped', 'delivered', 'cancelled']).describe('Where an order is in its lifecycle. The set is closed here; which value may FOLLOW which is the server\'s own lifecycle rules, answered per caller by `OrderActions`.').describe('Must be `processing`, `shipped` or `delivered`, and forward of the order\'s current status — every other value is a 409.'),
  "reason": zod.string().min(1).describe('Why the normal door didn\'t apply. Recorded on the order\'s override history, never optional.')
})
...
export const overrideOrderStatusResponseDataItemsItemLocaleRegExp = new RegExp('^[a-z]{2}(-[A-Za-z0-9]+)*$');
...
export const OverrideOrderStatusResponse = zod.strictObject({
  ...
  "id": zod.string().describe('Resource identifier'),
  "title": zod.string(),
  "price": zod.number().min(overrideOrderStatusResponseDataItemsItemProductPriceMin).describe('Gross — what the customer paid, VAT included, frozen at checkout. Same convention as `Product.price`.'),
  "description": zod.string().optional(),
  "active": zod.boolean().optional(),
  "requiresShipping": zod.boolean().default(overrideOrderStatusResponseDataItemsItemProductRequiresShippingDefault),
  "weight": zod.number().min(overrideOrderStatusResponseDataItemsItemProductWeightMin).optional(),
  "categories": zod.array(zod.string()).optional(),
  "tags": zod.array(zod.string()).optional(),
  "createdAt": zod.iso.datetime({"offset":true}).optional(),
  "updatedAt": zod.iso.datetime({"offset":true}).optional(),
  "deletedAt": zod.iso.datetime({"offset":true}).optional(),
  "taxRate": zod.number().min(overrideOrderStatusResponseDataItemsItemProductTaxRateMin).max(overrideOrderStatusResponseDataItemsItemProductTaxRateMax).optional().describe('The VAT rate this line was actually charged, as a decimal (0.22 for 22%). Frozen at checkout, same reasoning as `price`. Absent on an order placed before VAT existed.')
}),
  "quantity": zod.number().min(1),
  "locale": zod.string().regex(overrideOrderStatusResponseDataItemsItemLocaleRegExp).describe('BCP 47 language tag, e.g. `en` or `it`. Which tags a deployment actually supports is a runtime fact, not a contract one — ask `GET \/locales`.'),
  "current": zod.strictObject({
  "imageUrl": zod.string().describe('Absolute URL or server-relative upload path (e.g. `\/uploads\/abc.jpg`). `uri-reference`, not `uri`: an uploaded image is stored and returned as a path relative to the API host, which is not a valid absolute URI.'),
  "thumbnailUrl": zod.string().optional().describe('Server-relative path to a small WebP derivative of `imageUrl`, produced by the image digest pipeline once an uploaded image has finished processing (see `docs\/tools\/image-processing.md`). Absent for a record whose image is a remote or default URL rather than an upload — there is nothing to derive a thumbnail from. Never accepted on a request body: the server is the only writer.')
}).nullable().describe('The product\'s picture, resolved live — `null` when the catalogue product (`product.id`) has been hard-deleted. Never the terms of the sale, so it is never frozen; see `OrderLineCurrent`.'),
  "taxAmount": zod.number().min(overrideOrderStatusResponseDataItemsItemTaxAmountMin).optional().describe('VAT included in this line\'s total, at its own frozen `taxRate`.'),
  "netAmount": zod.number().min(overrideOrderStatusResponseDataItemsItemNetAmountMin).optional().describe('This line\'s total excluding VAT — `price × quantity` minus `taxAmount`.')
})),
  "totalItems": zod.number().min(overrideOrderStatusResponseDataTotalItemsMin).describe('Number of distinct line items in this order. Not to be confused with `PaginationMeta.totalItems`, which counts orders matching a search.'),
  "totalQuantity": zod.number().min(overrideOrderStatusResponseDataTotalQuantityMin).describe('Sum of `quantity` across every line item.'),
  "totalPrice": zod.number().min(overrideOrderStatusResponseDataTotalPriceMin).describe('Sum of `product.price × quantity` across every line item, plus `shippingCost` when the checkout chose a method.'),
  "netTotal": zod.number().min(overrideOrderStatusResponseDataNetTotalMin).optional().describe('Sum of every line\'s `netAmount` — the goods total excluding VAT, GOODS ONLY. Shipping\'s own net amount is `shippingNetAmount`, not folded in here. Absent on a pre-VAT order.'),
  "taxTotal": zod.number().min(overrideOrderStatusResponseDataTaxTotalMin).optional().describe('Every VAT collected on this order: the lines\' own `taxAmount`, plus the VAT on `shippingCost` — apportioned pro-rata across the lines by value and taxed at each line\'s own rate, since delivery is taxed as ancillary to what it delivers. Absent on a pre-VAT order.'),
  "shippingNetAmount": zod.number().min(overrideOrderStatusResponseDataShippingNetAmountMin).optional().describe('Shipping\'s own net amount, summed across every line it was apportioned onto — `netTotal` stays goods-only, this is the rest of the split. Absent on a pre-VAT order.'),
  "shippingTaxAmount": zod.number().min(overrideOrderStatusResponseDataShippingTaxAmountMin).optional().describe('Shipping\'s own tax amount, summed across every line it was apportioned onto — already folded into `taxTotal`, published separately so an invoice can print it as its own line. Absent on a pre-VAT order.'),
  "taxSummary": zod.array(zod.strictObject({
  "rate": zod.number().min(overrideOrderStatusResponseDataTaxSummaryItemRateMin).describe('The decimal rate this row is for (`0.22` for 22%) — never repeated across rows.'),
  "netAmount": zod.number().min(overrideOrderStatusResponseDataTaxSummaryItemNetAmountMin),
  "taxAmount": zod.number().min(overrideOrderStatusResponseDataTaxSummaryItemTaxAmountMin),
  "grossAmount": zod.number().min(overrideOrderStatusResponseDataTaxSummaryItemGrossAmountMin).describe('`netAmount + taxAmount`, not re-derived from a price.')
})).optional().describe('One row per distinct VAT rate charged on this order, sorted ascending. Reconciles exactly: summed `netAmount` is `netTotal` + `shippingNetAmount`, summed `taxAmount` is `taxTotal`, and summed `grossAmount` is `totalPrice`. Absent on a pre-VAT order.'),
  "notes": zod.string().optional().describe('Optional order notes'),
  "shippingMethod": zod.string().optional().describe('The shipping method\'s id as the checkout froze it (e.g. standard, express, pickup).'),
  "shippingCost": zod.number().min(overrideOrderStatusResponseDataShippingCostMin).optional().describe('What that method cost at checkout time — a later rate change cannot re-price history.'),
  "shippingAddress": zod.strictObject({
  "fullName": zod.string(),
  "street": zod.string(),
  "city": zod.string(),
...
export const ListPaymentMethodsResponse = zod.strictObject({
  ...
  "data": zod.strictObject({
  "methods": zod.array(zod.strictObject({
  "id": zod.enum(['card', 'bank_transfer']).describe('How an order is being paid for. A preference recorded at checkout, not a lock — a card payment still settles normally regardless of this value.'),
  "holdHours": zod.number().min(listPaymentMethodsResponseDataMethodsItemHoldHoursMin).optional().describe('How long checkout holds stock for an order choosing this method (`NODE_BANK_TRANSFER_HOLD_HOURS`). Present on `bank_transfer` only — `card`\'s hold is `NODE_RESERVATION_TTL_MINUTES`, a different unit, owned by `inventory` rather than a deployment choice this module makes.')
}))
})
})
...
export const CreatePaymentIntentHeader = zod.strictObject({
  "Idempotency-Key": zod.string().min(1).max(createPaymentIntentHeaderIdempotencyKeyMax).regex(createPaymentIntentHeaderIdempotencyKeyRegExp).optional().describe('An opaque, client-generated value (a UUID by convention) that makes a retried write safe. Repeating this request with the SAME key and the SAME body replays the first response (`Idempotent-Replay: true`, no repeated write) instead of running it again; the same key with a DIFFERENT body answers 422; a key still being processed by another in-flight request answers 409. Omitting the header simply forgoes replay protection — the write still happens normally.\n')
})
...
export const CreatePaymentIntentResponse = zod.strictObject({
  ...
  "provider": zod.string().describe('Which provider implementation handled it — `fake` in the demo, `manual` for a payment recorded by hand (`POST \/payments\/order\/{orderId}\/offline`), or a real PSP\'s name.'),
  "method": zod.enum(['card', 'bank_transfer', 'cash', 'other']).describe('How the money moved. `card` for anything that went through the provider port; the other three are set only by `POST \/payments\/order\/{orderId}\/offline`, from what the admin chose there.'),
  "reference": zod.string().max(createPaymentIntentResponseDataReferenceMax).optional().describe('Free text identifying an offline payment — a bank transaction id, a receipt number. Set only by `POST \/payments\/order\/{orderId}\/offline`; absent from a card payment.'),
  "receivedAt": zod.iso.datetime({"offset":true}).optional().describe('When the money actually arrived, as the admin recording it reported — possibly earlier than `createdAt`. Set only by `POST \/payments\/order\/{orderId}\/offline`; absent from a card payment.'),
  "refundedByHand": zod.boolean().optional().describe('`true` once a refunded `manual` payment has had its money returned to the customer outside this application — there is no provider to ask, so this is the admin\'s own record that it was done. Absent otherwise.'),
...
export const GetPaymentByOrderResponse = zod.strictObject({
  ...
  "provider": zod.string().describe('Which provider implementation handled it — `fake` in the demo, `manual` for a payment recorded by hand (`POST \/payments\/order\/{orderId}\/offline`), or a real PSP\'s name.'),
  "method": zod.enum(['card', 'bank_transfer', 'cash', 'other']).describe('How the money moved. `card` for anything that went through the provider port; the other three are set only by `POST \/payments\/order\/{orderId}\/offline`, from what the admin chose there.'),
  "reference": zod.string().max(getPaymentByOrderResponseDataReferenceMax).optional().describe('Free text identifying an offline payment — a bank transaction id, a receipt number. Set only by `POST \/payments\/order\/{orderId}\/offline`; absent from a card payment.'),
  "receivedAt": zod.iso.datetime({"offset":true}).optional().describe('When the money actually arrived, as the admin recording it reported — possibly earlier than `createdAt`. Set only by `POST \/payments\/order\/{orderId}\/offline`; absent from a card payment.'),
  "refundedByHand": zod.boolean().optional().describe('`true` once a refunded `manual` payment has had its money returned to the customer outside this application — there is no provider to ask, so this is the admin\'s own record that it was done. Absent otherwise.'),
  "clientSecret": zod.string().optional().describe('Returned by `POST \/payments\/intent` alone, never stored and never read back: it authorises completing this payment against the provider from the browser. Absent from every other response.'),
  "cardLast4": zod.string().optional().describe('The only card digits a payment system may remember. Survives a refund — refunding does not clear it.'),
...
export const GetOrderByReferenceQueryParams = zod.strictObject({
  "ref": zod.string().min(1).max(getOrderByReferenceQueryRefMax).describe('The RF reference, spaces and case tolerated exactly as a customer might type it — e.g. `RF13 2EY8 H44V JAVZ KX80 JRL` — or a raw 24-character order id for an order that predates this field.')
})
...
export const getOrderByReferenceResponseDataItemsItemLocaleRegExp = new RegExp('^[a-z]{2}(-[A-Za-z0-9]+)*$');
...
export const GetOrderByReferenceResponse = zod.strictObject({
  ...
  "id": zod.string().describe('Resource identifier'),
  "title": zod.string(),
  "price": zod.number().min(getOrderByReferenceResponseDataItemsItemProductPriceMin).describe('Gross — what the customer paid, VAT included, frozen at checkout. Same convention as `Product.price`.'),
  "description": zod.string().optional(),
  "active": zod.boolean().optional(),
  "requiresShipping": zod.boolean().default(getOrderByReferenceResponseDataItemsItemProductRequiresShippingDefault),
  "weight": zod.number().min(getOrderByReferenceResponseDataItemsItemProductWeightMin).optional(),
  "categories": zod.array(zod.string()).optional(),
  "tags": zod.array(zod.string()).optional(),
  "createdAt": zod.iso.datetime({"offset":true}).optional(),
  "updatedAt": zod.iso.datetime({"offset":true}).optional(),
  "deletedAt": zod.iso.datetime({"offset":true}).optional(),
  "taxRate": zod.number().min(getOrderByReferenceResponseDataItemsItemProductTaxRateMin).max(getOrderByReferenceResponseDataItemsItemProductTaxRateMax).optional().describe('The VAT rate this line was actually charged, as a decimal (0.22 for 22%). Frozen at checkout, same reasoning as `price`. Absent on an order placed before VAT existed.')
}),
  "quantity": zod.number().min(1),
  "locale": zod.string().regex(getOrderByReferenceResponseDataItemsItemLocaleRegExp).describe('BCP 47 language tag, e.g. `en` or `it`. Which tags a deployment actually supports is a runtime fact, not a contract one — ask `GET \/locales`.'),
  "current": zod.strictObject({
  "imageUrl": zod.string().describe('Absolute URL or server-relative upload path (e.g. `\/uploads\/abc.jpg`). `uri-reference`, not `uri`: an uploaded image is stored and returned as a path relative to the API host, which is not a valid absolute URI.'),
  "thumbnailUrl": zod.string().optional().describe('Server-relative path to a small WebP derivative of `imageUrl`, produced by the image digest pipeline once an uploaded image has finished processing (see `docs\/tools\/image-processing.md`). Absent for a record whose image is a remote or default URL rather than an upload — there is nothing to derive a thumbnail from. Never accepted on a request body: the server is the only writer.')
}).nullable().describe('The product\'s picture, resolved live — `null` when the catalogue product (`product.id`) has been hard-deleted. Never the terms of the sale, so it is never frozen; see `OrderLineCurrent`.'),
  "taxAmount": zod.number().min(getOrderByReferenceResponseDataItemsItemTaxAmountMin).optional().describe('VAT included in this line\'s total, at its own frozen `taxRate`.'),
  "netAmount": zod.number().min(getOrderByReferenceResponseDataItemsItemNetAmountMin).optional().describe('This line\'s total excluding VAT — `price × quantity` minus `taxAmount`.')
})),
  "totalItems": zod.number().min(getOrderByReferenceResponseDataTotalItemsMin).describe('Number of distinct line items in this order. Not to be confused with `PaginationMeta.totalItems`, which counts orders matching a search.'),
  "totalQuantity": zod.number().min(getOrderByReferenceResponseDataTotalQuantityMin).describe('Sum of `quantity` across every line item.'),
  "totalPrice": zod.number().min(getOrderByReferenceResponseDataTotalPriceMin).describe('Sum of `product.price × quantity` across every line item, plus `shippingCost` when the checkout chose a method.'),
  "netTotal": zod.number().min(getOrderByReferenceResponseDataNetTotalMin).optional().describe('Sum of every line\'s `netAmount` — the goods total excluding VAT, GOODS ONLY. Shipping\'s own net amount is `shippingNetAmount`, not folded in here. Absent on a pre-VAT order.'),
  "taxTotal": zod.number().min(getOrderByReferenceResponseDataTaxTotalMin).optional().describe('Every VAT collected on this order: the lines\' own `taxAmount`, plus the VAT on `shippingCost` — apportioned pro-rata across the lines by value and taxed at each line\'s own rate, since delivery is taxed as ancillary to what it delivers. Absent on a pre-VAT order.'),
  "shippingNetAmount": zod.number().min(getOrderByReferenceResponseDataShippingNetAmountMin).optional().describe('Shipping\'s own net amount, summed across every line it was apportioned onto — `netTotal` stays goods-only, this is the rest of the split. Absent on a pre-VAT order.'),
  "shippingTaxAmount": zod.number().min(getOrderByReferenceResponseDataShippingTaxAmountMin).optional().describe('Shipping\'s own tax amount, summed across every line it was apportioned onto — already folded into `taxTotal`, published separately so an invoice can print it as its own line. Absent on a pre-VAT order.'),
  "taxSummary": zod.array(zod.strictObject({
  "rate": zod.number().min(getOrderByReferenceResponseDataTaxSummaryItemRateMin).describe('The decimal rate this row is for (`0.22` for 22%) — never repeated across rows.'),
  "netAmount": zod.number().min(getOrderByReferenceResponseDataTaxSummaryItemNetAmountMin),
  "taxAmount": zod.number().min(getOrderByReferenceResponseDataTaxSummaryItemTaxAmountMin),
  "grossAmount": zod.number().min(getOrderByReferenceResponseDataTaxSummaryItemGrossAmountMin).describe('`netAmount + taxAmount`, not re-derived from a price.')
})).optional().describe('One row per distinct VAT rate charged on this order, sorted ascending. Reconciles exactly: summed `netAmount` is `netTotal` + `shippingNetAmount`, summed `taxAmount` is `taxTotal`, and summed `grossAmount` is `totalPrice`. Absent on a pre-VAT order.'),
  "notes": zod.string().optional().describe('Optional order notes'),
  "shippingMethod": zod.string().optional().describe('The shipping method\'s id as the checkout froze it (e.g. standard, express, pickup).'),
  "shippingCost": zod.number().min(getOrderByReferenceResponseDataShippingCostMin).optional().describe('What that method cost at checkout time — a later rate change cannot re-price history.'),
  "shippingAddress": zod.strictObject({
  "fullName": zod.string(),
  "street": zod.string(),
  "city": zod.string(),
...
export const RefundPaymentByOrderHeader = zod.strictObject({
  "Idempotency-Key": zod.string().min(1).max(refundPaymentByOrderHeaderIdempotencyKeyMax).regex(refundPaymentByOrderHeaderIdempotencyKeyRegExp).optional().describe('An opaque, client-generated value (a UUID by convention) that makes a retried write safe. Repeating this request with the SAME key and the SAME body replays the first response (`Idempotent-Replay: true`, no repeated write) instead of running it again; the same key with a DIFFERENT body answers 422; a key still being processed by another in-flight request answers 409. Omitting the header simply forgoes replay protection — the write still happens normally.\n')
})
...
export const RefundPaymentByOrderResponse = zod.strictObject({
  ...
  "provider": zod.string().describe('Which provider implementation handled it — `fake` in the demo, `manual` for a payment recorded by hand (`POST \/payments\/order\/{orderId}\/offline`), or a real PSP\'s name.'),
  "method": zod.enum(['card', 'bank_transfer', 'cash', 'other']).describe('How the money moved. `card` for anything that went through the provider port; the other three are set only by `POST \/payments\/order\/{orderId}\/offline`, from what the admin chose there.'),
  "reference": zod.string().max(refundPaymentByOrderResponseDataReferenceMax).optional().describe('Free text identifying an offline payment — a bank transaction id, a receipt number. Set only by `POST \/payments\/order\/{orderId}\/offline`; absent from a card payment.'),
  "receivedAt": zod.iso.datetime({"offset":true}).optional().describe('When the money actually arrived, as the admin recording it reported — possibly earlier than `createdAt`. Set only by `POST \/payments\/order\/{orderId}\/offline`; absent from a card payment.'),
  "refundedByHand": zod.boolean().optional().describe('`true` once a refunded `manual` payment has had its money returned to the customer outside this application — there is no provider to ask, so this is the admin\'s own record that it was done. Absent otherwise.'),
  "clientSecret": zod.string().optional().describe('Returned by `POST \/payments\/intent` alone, never stored and never read back: it authorises completing this payment against the provider from the browser. Absent from every other response.'),
  "cardLast4": zod.string().optional().describe('The only card digits a payment system may remember. Survives a refund — refunding does not clear it.'),
...
export const RecordOfflinePaymentHeader = zod.strictObject({
  "Idempotency-Key": zod.string().min(1).max(recordOfflinePaymentHeaderIdempotencyKeyMax).regex(recordOfflinePaymentHeaderIdempotencyKeyRegExp).optional().describe('An opaque, client-generated value (a UUID by convention) that makes a retried write safe. Repeating this request with the SAME key and the SAME body replays the first response (`Idempotent-Replay: true`, no repeated write) instead of running it again; the same key with a DIFFERENT body answers 422; a key still being processed by another in-flight request answers 409. Omitting the header simply forgoes replay protection — the write still happens normally.\n')
})
...
export const RecordOfflinePaymentBody = zod.strictObject({
  "method": zod.enum(['bank_transfer', 'cash', 'other']).describe('How the money arrived. `card` is not offered here — that path is `POST \/payments\/intent`.'),
  "reference": zod.string().max(recordOfflinePaymentBodyReferenceMax).optional().describe('A bank transaction id, a receipt number — whatever ties this record to the money.'),
  "receivedAt": zod.iso.datetime({"offset":true}).optional().describe('When the money arrived. Must not be in the future. Defaults to now.')
})
...
export const RecordOfflinePaymentResponse = zod.strictObject({
  ...
  "orderId": zod.string().describe('Resource identifier'),
  "userId": zod.string().optional().describe('Resource identifier'),
  "amount": zod.number().min(recordOfflinePaymentResponseDataAmountMin).describe('The order\'s total as the intent froze it. Always two decimal places, rounded half-up at the point of calculation.'),
  "currency": zod.string().describe('ISO-4217 currency code (e.g. EUR)'),
  "status": zod.enum(['requires_confirmation', 'requires_action', 'processing', 'succeeded', 'declined', 'refunded']).describe('The provider-facing lifecycle. `requires_action` means the bank wants a challenge answered in the browser (3-D Secure) and `processing` that the provider has taken the payment but not settled it — both are in flight, and `POST \/payments\/{id}\/sync` is what resolves them without waiting for the webhook. `declined` is retryable: the confirm endpoint accepts the same payment again with another method. `refunded` is terminal. Full transition table: docs\/modules\/payments.md#status-transitions'),
  "provider": zod.string().describe('Which provider implementation handled it — `fake` in the demo, `manual` for a payment recorded by hand (`POST \/payments\/order\/{orderId}\/offline`), or a real PSP\'s name.'),
  "method": zod.enum(['card', 'bank_transfer', 'cash', 'other']).describe('How the money moved. `card` for anything that went through the provider port; the other three are set only by `POST \/payments\/order\/{orderId}\/offline`, from what the admin chose there.'),
  "reference": zod.string().max(recordOfflinePaymentResponseDataReferenceMax).optional().describe('Free text identifying an offline payment — a bank transaction id, a receipt number. Set only by `POST \/payments\/order\/{orderId}\/offline`; absent from a card payment.'),
  "receivedAt": zod.iso.datetime({"offset":true}).optional().describe('When the money actually arrived, as the admin recording it reported — possibly earlier than `createdAt`. Set only by `POST \/payments\/order\/{orderId}\/offline`; absent from a card payment.'),
  "refundedByHand": zod.boolean().optional().describe('`true` once a refunded `manual` payment has had its money returned to the customer outside this application — there is no provider to ask, so this is the admin\'s own record that it was done. Absent otherwise.'),
  "clientSecret": zod.string().optional().describe('Returned by `POST \/payments\/intent` alone, never stored and never read back: it authorises completing this payment against the provider from the browser. Absent from every other response.'),
  "cardLast4": zod.string().optional().describe('The only card digits a payment system may remember. Survives a refund — refunding does not clear it.'),
...
export const ConfirmPaymentHeader = zod.strictObject({
  "Idempotency-Key": zod.string().min(1).max(confirmPaymentHeaderIdempotencyKeyMax).regex(confirmPaymentHeaderIdempotencyKeyRegExp).optional().describe('An opaque, client-generated value (a UUID by convention) that makes a retried write safe. Repeating this request with the SAME key and the SAME body replays the first response (`Idempotent-Replay: true`, no repeated write) instead of running it again; the same key with a DIFFERENT body answers 422; a key still being processed by another in-flight request answers 409. Omitting the header simply forgoes replay protection — the write still happens normally.\n')
})
...
export const ConfirmPaymentResponse = zod.strictObject({
  ...
  "provider": zod.string().describe('Which provider implementation handled it — `fake` in the demo, `manual` for a payment recorded by hand (`POST \/payments\/order\/{orderId}\/offline`), or a real PSP\'s name.'),
  "method": zod.enum(['card', 'bank_transfer', 'cash', 'other']).describe('How the money moved. `card` for anything that went through the provider port; the other three are set only by `POST \/payments\/order\/{orderId}\/offline`, from what the admin chose there.'),
  "reference": zod.string().max(confirmPaymentResponseDataReferenceMax).optional().describe('Free text identifying an offline payment — a bank transaction id, a receipt number. Set only by `POST \/payments\/order\/{orderId}\/offline`; absent from a card payment.'),
  "receivedAt": zod.iso.datetime({"offset":true}).optional().describe('When the money actually arrived, as the admin recording it reported — possibly earlier than `createdAt`. Set only by `POST \/payments\/order\/{orderId}\/offline`; absent from a card payment.'),
  "refundedByHand": zod.boolean().optional().describe('`true` once a refunded `manual` payment has had its money returned to the customer outside this application — there is no provider to ask, so this is the admin\'s own record that it was done. Absent otherwise.'),
  "clientSecret": zod.string().optional().describe('Returned by `POST \/payments\/intent` alone, never stored and never read back: it authorises completing this payment against the provider from the browser. Absent from every other response.'),
  "cardLast4": zod.string().optional().describe('The only card digits a payment system may remember. Survives a refund — refunding does not clear it.'),
...
export const SyncPaymentResponse = zod.strictObject({
  ...
  "provider": zod.string().describe('Which provider implementation handled it — `fake` in the demo, `manual` for a payment recorded by hand (`POST \/payments\/order\/{orderId}\/offline`), or a real PSP\'s name.'),
  "method": zod.enum(['card', 'bank_transfer', 'cash', 'other']).describe('How the money moved. `card` for anything that went through the provider port; the other three are set only by `POST \/payments\/order\/{orderId}\/offline`, from what the admin chose there.'),
  "reference": zod.string().max(syncPaymentResponseDataReferenceMax).optional().describe('Free text identifying an offline payment — a bank transaction id, a receipt number. Set only by `POST \/payments\/order\/{orderId}\/offline`; absent from a card payment.'),
  "receivedAt": zod.iso.datetime({"offset":true}).optional().describe('When the money actually arrived, as the admin recording it reported — possibly earlier than `createdAt`. Set only by `POST \/payments\/order\/{orderId}\/offline`; absent from a card payment.'),
  "refundedByHand": zod.boolean().optional().describe('`true` once a refunded `manual` payment has had its money returned to the customer outside this application — there is no provider to ask, so this is the admin\'s own record that it was done. Absent otherwise.'),
  "clientSecret": zod.string().optional().describe('Returned by `POST \/payments\/intent` alone, never stored and never read back: it authorises completing this payment against the provider from the browser. Absent from every other response.'),
  "cardLast4": zod.string().optional().describe('The only card digits a payment system may remember. Survives a refund — refunding does not clear it.'),
...
export const ListShippingMethodsQueryParams = zod.strictObject({
  "weight": zod.number().min(listShippingMethodsQueryWeightMin).optional().describe('The current basket\'s total weight in grams, computed by the caller. Omitted returns every method regardless of weight range.')
})
...
export const ListShippingMethodsResponse = zod.strictObject({
  ...
  "tracked": zod.boolean().describe('Whether a parcel sent by this method carries a tracking code. Looked up live by the order\'s shippingMethod id when it ships — not frozen at checkout — so a rate change also changes what the shipping door requires.'),
  "maxInsuredValue": zod.number().min(listShippingMethodsResponseDataMethodsItemMaxInsuredValueMin).optional().describe('The most this method insures a parcel for. Informational only — nothing in this application enforces it against an order\'s total.'),
  "minWeight": zod.number().min(listShippingMethodsResponseDataMethodsItemMinWeightMin).optional().describe('Grams. Absent means no floor. Enforced — GET \/delivery\/methods?weight= omits a method the basket doesn\'t reach, and checkout refuses one chosen outside its range.'),
  "maxWeight": zod.number().min(listShippingMethodsResponseDataMethodsItemMaxWeightMin).optional().describe('Grams. Absent means no ceiling. Enforced the same way as minWeight.')
}))
})
})
...
export const GetShipmentByOrderResponse = zod.strictObject({
  ...
  "id": zod.string().describe('Resource identifier'),
  "orderId": zod.string().describe('Resource identifier'),
  "trackingCode": zod.string().optional().describe('The courier\'s handle on the parcel. Absent for a method that carries no tracking (ShippingMethod.tracked is false).'),
...
export const ShipOrderBody = zod.strictObject({
  "trackingCode": zod.string().optional().describe('The carrier\'s handle on the parcel. Required when the order\'s shipping method is tracked; optional otherwise.'),
  "forced": zod.boolean().optional().describe('Skip the normal `processing`-only gate — the caller must hold `orders.any.override`, and `reason` is then required. Still creates the parcel record and sends the shipped email; still refuses a tracked method with no `trackingCode`.'),
  "reason": zod.string().min(1).optional().describe('Required exactly when `forced` is `true` — why the normal door didn\'t apply. Recorded on the order\'s override history.')
})
...
export const DeliverOrderBody = zod.strictObject({
  "forced": zod.boolean().optional().describe('Skip the normal `shipped`-only gate — the caller must hold `orders.any.override`, and `reason` is then required. Still stamps the shipment delivered and reports the fact.'),
...
export const ListWebhookSubscriptionsQueryParams = zod.strictObject({
  "page": zod.number().min(1).max(listWebhookSubscriptionsQueryPageMax).default(listWebhookSubscriptionsQueryPageDefault).describe('1-based page index'),
  "pageSize": zod.number().min(1).max(listWebhookSubscriptionsQueryPageSizeMax).default(listWebhookSubscriptionsQueryPageSizeDefault),
  "enabled": zod.boolean().optional().describe('Filter by whether the subscription is currently active.')
})
...
export const ListWebhookSubscriptionsResponse = zod.strictObject({
  ...
  "url": zod.url(),
  "description": zod.string().optional(),
  "eventTypes": zod.array(zod.string()).min(1).describe('Names from `GET \/webhooks\/events` this subscription receives.'),
  "enabled": zod.boolean(),
  "consecutiveFailures": zod.number().min(listWebhookSubscriptionsResponseDataItemsItemConsecutiveFailuresMin),
  "disabledAt": zod.iso.datetime({"offset":true}).optional().describe('Set once `consecutiveFailures` crosses the auto-disable threshold.'),
  "secretIds": zod.array(zod.string()).describe('The ring\'s secret ids, oldest first — never the plaintext.'),
  "createdAt": zod.iso.datetime({"offset":true}),
  "updatedAt": zod.iso.datetime({"offset":true})
})),
  "meta": zod.strictObject({
  "page": zod.number().min(1).max(listWebhookSubscriptionsResponseDataMetaPageMax).default(listWebhookSubscriptionsResponseDataMetaPageDefault).describe('1-based page index. Bounded so page × pageSize cannot ask for an unbounded Mongo skip.'),
  "pageSize": zod.number().min(1).max(listWebhookSubscriptionsResponseDataMetaPageSizeMax).default(listWebhookSubscriptionsResponseDataMetaPageSizeDefault).describe('Optional override; server may clamp to a max'),
...
export const CreateWebhookSubscriptionBody = zod.strictObject({
  "url": zod.url().describe('Must be `https:\/\/`. Validated again, against the resolved IP, on every delivery.'),
  "description": zod.string().optional(),
...
export const CreateWebhookSubscriptionResponse = zod.strictObject({
  ...
  "eventTypes": zod.array(zod.string()).min(1),
  "enabled": zod.boolean(),
  "consecutiveFailures": zod.number().min(createWebhookSubscriptionResponseDataConsecutiveFailuresMin),
  "disabledAt": zod.iso.datetime({"offset":true}).optional(),
  "secretIds": zod.array(zod.string()),
  "createdAt": zod.iso.datetime({"offset":true}),
  "updatedAt": zod.iso.datetime({"offset":true}),
  "secret": zod.string().optional().describe('The newly minted secret, in plaintext. Shown here once, on creation, and never again.'),
  "newSecret": zod.string().optional().describe('A rotated-in secret\'s plaintext, present only when `PATCH` was sent with `rotateSecret:true`.')
})
})
...
export const UpdateWebhookSubscriptionBody = zod.strictObject({
  ...
  "enabled": zod.boolean().optional().describe('Setting this true re-arms a subscription the auto-disable guard turned off, and clears `disabledAt`.'),
  "rotateSecret": zod.boolean().optional().describe('Add a new secret to the ring; its plaintext comes back once, in `newSecret`.'),
  "removeSecretId": zod.string().optional().describe('Drop this secret id from the ring — the other half of a rotation, once every consumer has switched.')
})
...
export const UpdateWebhookSubscriptionResponse = zod.strictObject({
  ...
  "eventTypes": zod.array(zod.string()).min(1),
  "enabled": zod.boolean(),
  "consecutiveFailures": zod.number().min(updateWebhookSubscriptionResponseDataConsecutiveFailuresMin),
  "disabledAt": zod.iso.datetime({"offset":true}).optional(),
  "secretIds": zod.array(zod.string()),
  "createdAt": zod.iso.datetime({"offset":true}),
  "updatedAt": zod.iso.datetime({"offset":true}),
...
export const ListWebhookDeliveriesQueryParams = zod.strictObject({
  "page": zod.number().min(1).max(listWebhookDeliveriesQueryPageMax).default(listWebhookDeliveriesQueryPageDefault).describe('1-based page index'),
  "pageSize": zod.number().min(1).max(listWebhookDeliveriesQueryPageSizeMax).default(listWebhookDeliveriesQueryPageSizeDefault),
...
export const ListWebhookDeliveriesResponse = zod.strictObject({
  ...
  "meta": zod.strictObject({
  "page": zod.number().min(1).max(listWebhookDeliveriesResponseDataMetaPageMax).default(listWebhookDeliveriesResponseDataMetaPageDefault).describe('1-based page index. Bounded so page × pageSize cannot ask for an unbounded Mongo skip.'),
  "pageSize": zod.number().min(1).max(listWebhookDeliveriesResponseDataMetaPageSizeMax).default(listWebhookDeliveriesResponseDataMetaPageSizeDefault).describe('Optional override; server may clamp to a max'),
...
export const ListApiKeysQueryParams = zod.strictObject({
  "page": zod.number().min(1).max(listApiKeysQueryPageMax).default(listApiKeysQueryPageDefault).describe('1-based page index'),
  "pageSize": zod.number().min(1).max(listApiKeysQueryPageSizeMax).default(listApiKeysQueryPageSizeDefault)
})
...
export const ListApiKeysResponse = zod.strictObject({
  ...
  "meta": zod.strictObject({
  "page": zod.number().min(1).max(listApiKeysResponseDataMetaPageMax).default(listApiKeysResponseDataMetaPageDefault).describe('1-based page index. Bounded so page × pageSize cannot ask for an unbounded Mongo skip.'),
  "pageSize": zod.number().min(1).max(listApiKeysResponseDataMetaPageSizeMax).default(listApiKeysResponseDataMetaPageSizeDefault).describe('Optional override; server may clamp to a max'),
...
export const MintApiKeyBody = zod.strictObject({
  "name": zod.string().min(1).max(mintApiKeyBodyNameMax).describe('A caller-chosen label — what this credential is for, shown back in the list.'),
  "permissions": zod.array(zod.string()).min(1).describe('The permission keys to mint this credential with. Must be a subset of the caller\'s own current permissions, or the request is refused with 422.'),
  "expiresAt": zod.iso.datetime({"offset":true}).optional().describe('Optional. Once past, the credential is refused exactly like a revoked one.')
})
...
export const MintApiKeyResponse = zod.strictObject({
  ...
  "createdAt": zod.iso.datetime({"offset":true}),
  "updatedAt": zod.iso.datetime({"offset":true}),
  "secret": zod.string().describe('The newly minted credential, in plaintext — `sk_<prefix>_<secret>`. Shown here once, on creation, and never again.')
})
})
